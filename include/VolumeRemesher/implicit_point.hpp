    ...
    T& lambda_d)
{
    ...
    const T conn = (pxqd * r[3] - r[0] * pdqd) * a2233 + (pzqd * r[3] - r[2] * pdqd) * a2132 -
                   (pyqd * r[3] - r[1] * pdqd) * a2133;
    ...
    T& lambda_d)
{
    ...
    lambda_x = nvwu * (nwy0 * nuz0 - nwz0 * nuy0) + nuvw * (nvy0 * nwz0 - nvz0 * nwy0) -
               nwvu * (nvy0 * nuz0 - nvz0 * nuy0);
    lambda_y = nwvu * (nvx0 * nuz0 - nvz0 * nux0) - nvwu * (nwx0 * nuz0 - nwz0 * nux0) -
               nuvw * (nvx0 * nwz0 - nvz0 * nwx0);
    lambda_z = nuvw * (nvx0 * nwy0 - nvy0 * nwx0) + nvwu * (nwx0 * nuy0 - nwy0 * nux0) -
               nwvu * (nvx0 * nuy0 - nvy0 * nux0);
    lambda_d = (nvx0 * (nwy0 * nuz0 - nwz0 * nuy0) + nvz0 * (nwx0 * nuy0 - nwy0 * nux0) -
                nvy0 * (nwx0 * nuz0 - nwz0 * nux0)) *
               v1[3] * w1[3] * u1[3];
...
inline int
genericPoint::dotProductSign2D(const genericPoint& a, const genericPoint& b, const genericPoint& c)
{
...
inline int
genericPoint::dotProductSign3D(const genericPoint& a, const genericPoint& b, const genericPoint& c)
{
...
inline int
genericPoint::orient2D(const genericPoint& a, const genericPoint& b, const genericPoint& c)
{
...
{
    return orient2dxy_indirect_IEE(
        a,
        b.toExplicit3D().X(),
        b.toExplicit3D().Y(),
        c.toExplicit3D().X(),
...
inline int
genericPoint::orient2Dxy(const genericPoint& a, const genericPoint& b, const genericPoint& c)
{
...
{
    return orient2dyz_indirect_IEE(
        a,
        b.toExplicit3D().Y(),
        b.toExplicit3D().Z(),
        c.toExplicit3D().Y(),
...
inline int
genericPoint::orient2Dyz(const genericPoint& a, const genericPoint& b, const genericPoint& c)
{
...
{
    return orient2dzx_indirect_IEE(
        a,
        b.toExplicit3D().Z(),
        b.toExplicit3D().X(),
        c.toExplicit3D().Z(),
...
inline int
genericPoint::orient2Dzx(const genericPoint& a, const genericPoint& b, const genericPoint& c)
{
...
inline int orient3d_IIIE(
    const genericPoint& a,
    const genericPoint& b,
    const genericPoint& c,
    const genericPoint& d)
{
    return orient3d_indirect_IIIE(
        a,
        b,
        c,
        d.toExplicit3D().X(),
...
inline int genericPoint::orient3D(
    const genericPoint& a,
    const genericPoint& b,
    const genericPoint& c,
    const genericPoint& d)
...
inline int genericPoint::inSphere(
    const genericPoint& a,
    const genericPoint& b,
    const genericPoint& c,
    const genericPoint& d,
    const genericPoint& e)
{
    const int num_explicit = a.isExplicit3D() + b.isExplicit3D() + c.isExplicit3D() +
                             d.isExplicit3D() + e.isExplicit3D();
    ...
    const genericPoint& e)
{
    ...
    const int idx = (a.isExplicit3D() << 4) + (b.isExplicit3D() << 3) + (c.isExplicit3D() << 2) +
                    (d.isExplicit3D() << 1) + e.isExplicit3D();
    const int* cfg = is_lookup + idx * 6;

    if (num_explicit == 4)
        return cfg[0] * inSphere_IEEEE(*A[cfg[1]], *A[cfg[2]], *A[cfg[3]], *A[cfg[4]], *A[cfg[5]]);
    if (num_explicit == 3)
        return cfg[0] * inSphere_IIEEE(*A[cfg[1]], *A[cfg[2]], *A[cfg[3]], *A[cfg[4]], *A[cfg[5]]);
    if (num_explicit == 2)
        return cfg[0] * inSphere_IIIEE(*A[cfg[1]], *A[cfg[2]], *A[cfg[3]], *A[cfg[4]], *A[cfg[5]]);
    if (num_explicit == 1)
        return cfg[0] * inSphere_IIIIE(*A[cfg[1]], *A[cfg[2]], *A[cfg[3]], *A[cfg[4]], *A[cfg[5]]);
    return cfg[0] * inSphere_IIIII(*A[cfg[1]], *A[cfg[2]], *A[cfg[3]], *A[cfg[4]], *A[cfg[5]]);
}
#endif


inline int inGabrielSphere_EEEE(
    const genericPoint& a,
    const genericPoint& b,
    const genericPoint& c,
    const genericPoint& d)
{
    return inGabrielSphere(
        a.toExplicit3D().X(),
        a.toExplicit3D().Y(),
        a.toExplicit3D().Z(),
        b.toExplicit3D().X(),
...
inline int genericPoint::inGabrielSphere(
    const genericPoint& a,
    const genericPoint& b,
    const genericPoint& c,
    const genericPoint& d)
...
inline int incircle2d_IEEE(
    const genericPoint& a,
    const genericPoint& b,
    const genericPoint& c,
    const genericPoint& d)
{
    return incircle_indirect_IEEE(
        a,
        b.toExplicit2D().X(),
        b.toExplicit2D().Y(),
        c.toExplicit2D().X(),
...
inline int incircle2d_IIEE(
    const genericPoint& a,
    const genericPoint& b,
    const genericPoint& c,
    const genericPoint& d)
{
    return incircle_indirect_IIEE(
        a,
        b,
        c.toExplicit2D().X(),
        c.toExplicit2D().Y(),
...
inline int genericPoint::incircle(
    const genericPoint& a,
    const genericPoint& b,
    const genericPoint& c,
    const genericPoint& d)
...
inline int incircle2dxy_IEEE(
    const genericPoint& a,
    const genericPoint& b,
    const genericPoint& c,
    const genericPoint& d)
{
    return incirclexy_indirect_IEEE(
        a,
        b.toExplicit3D().X(),
        b.toExplicit3D().Y(),
        c.toExplicit3D().X(),
...
inline int incircle2dxy_IIEE(
    const genericPoint& a,
    const genericPoint& b,
    const genericPoint& c,
    const genericPoint& d)
{
    return incirclexy_indirect_IIEE(
        a,
        b,
        c.toExplicit3D().X(),
        c.toExplicit3D().Y(),
...
inline int genericPoint::incirclexy(
    const genericPoint& a,
    const genericPoint& b,
    const genericPoint& c,
    const genericPoint& d)
...
inline bool
genericPoint::getIntervalLambda(interval_number& lx, interval_number& ly, interval_number& d) const
{
    if (isExplicit2D()) {
...
inline bool genericPoint::getIntervalLambda(
    interval_number& lx,
    interval_number& ly,
    interval_number& lz,
    interval_number& d) const
{
    if (isExplicit3D()) {
        ...
        return toBPT().getIntervalLambda(lx, ly, lz, d);
    else {
        assert(isTBC());
...
inline void genericPoint::getBigfloatLambda(bigfloat& lx, bigfloat& ly, bigfloat& lz, bigfloat& d)
    const
{
    if (isExplicit3D()) {
        const explicitPoint3D& e = toExplicit3D();
        ...
        toBPT().getBigfloatLambda(lx, ly, lz, d);
    else {
        assert(isTBC());
...
inline bool implicitPoint2D_SSI::getIntervalLambda(
    interval_number& lx,
    interval_number& ly,
    interval_number& d) const
{
...
inline implicitPoint2D_SSI::implicitPoint2D_SSI(
    const genericPoint& l11,
    const genericPoint& l12,
    const genericPoint& l21,
    const genericPoint& l22)
    : genericPoint(Point_Type::SSI)
    , l1_1(l11)
    , l1_2(l12)
    , l2_1(l21)
    , l2_2(l22)
{
    ...
    lambda2d_SSI<interval_number, interval_number>(
        l1,
        l2,
        m1,
        m2,
...
inline bool implicitPoint3D_LPI::getIntervalLambda(
    interval_number& lx,
    interval_number& ly,
    interval_number& lz,
    interval_number& d) const
...
inline implicitPoint3D_LPI::implicitPoint3D_LPI(
    const genericPoint& _p,
    const genericPoint& _q,
    const genericPoint& _r,
    const genericPoint& _s,
    const genericPoint& _t)
    : genericPoint(Point_Type::LPI)
    , ip(_p)
    , iq(_q)
    , ir(_r)
    , is(_s)
    , it(_t)
{
    ...
    lambda3d_LPI<interval_number, interval_number>(
        p,
        q,
        r,
        s,
...
inline bool implicitPoint3D_TPI::getIntervalLambda(
    interval_number& lx,
    interval_number& ly,
    interval_number& lz,
    interval_number& d) const
...
inline implicitPoint3D_TPI::implicitPoint3D_TPI(
    const genericPoint& _v1,
    const genericPoint& _v2,
    const genericPoint& _v3,
    const genericPoint& _w1,
    const genericPoint& _w2,
    const genericPoint& _w3,
    const genericPoint& _u1,
    const genericPoint& _u2,
    const genericPoint& _u3)
    : genericPoint(Point_Type::TPI)
    , iv1(_v1)
    , iv2(_v2)
    , iv3(_v3)
    , iw1(_w1)
...
inline bool implicitPoint3D_LNC::getIntervalLambda(
    interval_number& lx,
    interval_number& ly,
    interval_number& lz,
    interval_number& d) const
...
inline implicitPoint3D_LNC::implicitPoint3D_LNC(
    const genericPoint& _p,
    const genericPoint& _q,
    const double _t)
    : genericPoint(Point_Type::LNC)
    , ip(_p)
    , iq(_q)
    , t(_t)
{
    ...
    ip.getIntervalLambda(p[0], p[1], p[2], p[3]);
    iq.getIntervalLambda(q[0], q[1], q[2], q[3]);
    lambda3d_LNC<interval_number, interval_number, interval_number>(
        p,
        q,
        t,
        dfilter_lambda_x,
...
inline bool implicitPoint3D_BPT::getIntervalLambda(
    interval_number& lx,
    interval_number& ly,
    interval_number& lz,
    interval_number& d) const
...
inline implicitPoint3D_BPT::implicitPoint3D_BPT(
    const genericPoint& _p,
    const genericPoint& _q,
    const genericPoint& _r,
    const double _v,
    const double _u)
    : genericPoint(Point_Type::BPT)
    , ip(_p)
    , iq(_q)
    , ir(_r)
    , v(_v)
    , u(_u)
{
    ...
    iq.getIntervalLambda(q[0], q[1], q[2], q[3]);
    ir.getIntervalLambda(r[0], r[1], r[2], r[3]);
    lambda3d_BPT<interval_number, interval_number, interval_number>(
        p,
        q,
        r,
        u,
...
inline bool implicitPoint3D_TBC::getIntervalLambda(
    interval_number& lx,
    interval_number& ly,
    interval_number& lz,
    interval_number& d) const
...
inline implicitPoint3D_TBC::implicitPoint3D_TBC(
    const genericPoint& _p,
    const genericPoint& _q,
    const genericPoint& _r,
    const genericPoint& _s)
    : genericPoint(Point_Type::TBC)
    , ip(_p)
    , iq(_q)
    , ir(_r)
    , is(_s)
{
    ...
    ir.getIntervalLambda(r[0], r[1], r[2], r[3]);
    is.getIntervalLambda(s[0], s[1], s[2], s[3]);
    lambda3d_TBC<interval_number, interval_number>(
        p,
        q,
        r,
        s,
...
inline void
genericPoint::getExpansionLambda(expansion& lx, expansion& ly, expansion& lz, expansion& d) const
{
    if (isExplicit3D()) {
        const explicitPoint3D& e = toExplicit3D();
        lx = e.X(), ly = e.Y(), lz = e.Z(), d = 1;
    } else if (isLPI())
        toLPI().getExpansionLambda(lx, ly, lz, d);
    else if (isTPI())
        toTPI().getExpansionLambda(lx, ly, lz, d);
    else if (isLNC())
        toLNC().getExpansionLambda(lx, ly, lz, d);
    else if (isBPT())
        toBPT().getExpansionLambda(lx, ly, lz, d);
    else {
        assert(isTBC());
...
inline void implicitPoint2D_SSI::getExpansionLambda(expansion& lx, expansion& ly, expansion& d)
    const
{
...
inline void implicitPoint3D_LPI::getExpansionLambda(
    expansion& lx,
    expansion& ly,
    expansion& lz,
    expansion& d) const
...
inline void implicitPoint3D_TPI::getExpansionLambda(
    expansion& lx,
    expansion& ly,
    expansion& lz,
    expansion& d) const
{
    if (iv1.isExplicit3D() && iv2.isExplicit3D() && iv3.isExplicit3D() && iw1.isExplicit3D() &&
        iw2.isExplicit3D() && iw3.isExplicit3D() && iu1.isExplicit3D() && iu2.isExplicit3D() &&
        iu3.isExplicit3D()) {
        const explicitPoint3D &ev1 = iv1.toExplicit3D(), &ev2 = iv2.toExplicit3D(),
                              &ev3 = iv3.toExplicit3D();
        const explicitPoint3D &ew1 = iw1.toExplicit3D(), &ew2 = iw2.toExplicit3D(),
                              &ew3 = iw3.toExplicit3D();
        const explicitPoint3D &eu1 = iu1.toExplicit3D(), &eu2 = iu2.toExplicit3D(),
                              &eu3 = iu3.toExplicit3D();
...
inline void implicitPoint3D_LNC::getExpansionLambda(
    expansion& lx,
    expansion& ly,
    expansion& lz,
    expansion& d) const
...
inline void implicitPoint3D_BPT::getExpansionLambda(
    expansion& lx,
    expansion& ly,
    expansion& lz,
    expansion& d) const
...
inline void implicitPoint3D_TBC::getExpansionLambda(
    expansion& lx,
    expansion& ly,
    expansion& lz,
    expansion& d) const
{
    if (ip.isExplicit3D() && iq.isExplicit3D() && ir.isExplicit3D() && is.isExplicit3D()) {
        const explicitPoint3D &ep = ip.toExplicit3D(), &eq = iq.toExplicit3D(),
                              &er = ir.toExplicit3D(), &es = is.toExplicit3D();
        const s_expansion p[4] = {ep.X(), ep.Y(), ep.Z(), 1};
        const s_expansion q[4] = {eq.X(), eq.Y(), eq.Z(), 1};
        const s_expansion r[4] = {er.X(), er.Y(), er.Z(), 1};
        const s_expansion s[4] = {es.X(), es.Y(), es.Z(), 1};
...
inline void
implicitPoint3D_LPI::getBigfloatLambda(bigfloat& lx, bigfloat& ly, bigfloat& lz, bigfloat& d) const
{
...
inline void
implicitPoint3D_TPI::getBigfloatLambda(bigfloat& lx, bigfloat& ly, bigfloat& lz, bigfloat& d) const
{
...
inline void
implicitPoint3D_LNC::getBigfloatLambda(bigfloat& lx, bigfloat& ly, bigfloat& lz, bigfloat& d) const
{
    bigfloat p[4], q[4];
    ip.getBigfloatLambda(p[0], p[1], p[2], p[3]);
    iq.getBigfloatLambda(q[0], q[1], q[2], q[3]);
...
inline void
implicitPoint3D_BPT::getBigfloatLambda(bigfloat& lx, bigfloat& ly, bigfloat& lz, bigfloat& d) const
{
...
inline void
implicitPoint3D_TBC::getBigfloatLambda(bigfloat& lx, bigfloat& ly, bigfloat& lz, bigfloat& d) const
{
...
inline bool genericPoint::getExactXYZCoordinates(bigrational& x, bigrational& y, bigrational& z)
    const
{
    if (isExplicit3D())
        ...
        return toBPT().getExactXYZCoordinates(x, y, z);
    else if (isTBC())
        return toTBC().getExactXYZCoordinates(x, y, z);
...
inline bool
implicitPoint3D_LPI::getExactXYZCoordinates(bigrational& x, bigrational& y, bigrational& z) const
{
    bigfloat lx, ly, lz, d;
...
inline bool
implicitPoint3D_TPI::getExactXYZCoordinates(bigrational& x, bigrational& y, bigrational& z) const
{
    bigfloat lx, ly, lz, d;
    getBigfloatLambda(lx, ly, lz, d);
    if (sgn(d) == 0) return false;
...
inline bool
implicitPoint3D_LNC::getExactXYZCoordinates(bigrational& x, bigrational& y, bigrational& z) const
{
    bigfloat lx, ly, lz, d;
    getBigfloatLambda(lx, ly, lz, d);
    x = bigrational(lx);
...
inline bool
implicitPoint3D_BPT::getExactXYZCoordinates(bigrational& x, bigrational& y, bigrational& z) const
{
    bigfloat lx, ly, lz, d;
    getBigfloatLambda(lx, ly, lz, d);
    x = bigrational(lx);
...
inline bool
implicitPoint3D_TBC::getExactXYZCoordinates(bigrational& x, bigrational& y, bigrational& z) const
{
    bigfloat lx, ly, lz, d;
    getBigfloatLambda(lx, ly, lz, d);
    x = bigrational(lx);
...
inline int maxComponentInTriangleNormal_filtered(
    double ov1x,
    double ov1y,
    double ov1z,
    double ov2x,
...
inline int maxComponentInTriangleNormal_exact(
    double ov1x,
    double ov1y,
    double ov1z,
    double ov2x,
...
inline int genericPoint::maxComponentInTriangleNormal(
    double ov1x,
    double ov1y,
    double ov1z,
    double ov2x,
...
inline bool genericPoint::innerSegmentsCross(
    const genericPoint& A,
    const genericPoint& B,
...
inline bool genericPoint::segmentsCross(
    const genericPoint& A,
    const genericPoint& B,
    const genericPoint& P,
    const genericPoint& Q)
...
inline bool genericPoint::pointInInnerSegment(
    const genericPoint& p,
    const genericPoint& v1,
    const genericPoint& v2)
{
...
inline bool
genericPoint::pointInSegment(const genericPoint& p, const genericPoint& v1, const genericPoint& v2)
{
    if (misaligned(p, v1, v2)) return false;
...
inline bool genericPoint::pointInTriangle(
    const genericPoint& P,
    const genericPoint& A,
    const genericPoint& B,
    const genericPoint& C)
...
inline bool genericPoint::pointInInnerTriangle(
    const genericPoint& P,
    const genericPoint& A,
    const genericPoint& B,
    const genericPoint& C)
...
inline bool genericPoint::lineCrossesInnerTriangle(
    const genericPoint& s1,
    const genericPoint& s2,
    const genericPoint& v1,
    const genericPoint& v2,
...
inline bool genericPoint::lineCrossesTriangle(
    const genericPoint& s1,
    const genericPoint& s2,
    const genericPoint& v1,
    const genericPoint& v2,
...
inline bool genericPoint::innerSegmentCrossesTriangle(
    const genericPoint& s1,
    const genericPoint& s2,
    const genericPoint& v1,
    const genericPoint& v2,
...
inline bool genericPoint::pointInSegment(
    const genericPoint& p,
    const genericPoint& v1,
    const genericPoint& v2,
    int xyz)
