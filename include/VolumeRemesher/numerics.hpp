...
interval_number::operator*(const interval_number &b) const {
  ...
  case 1:
    return interval_number(_mm_mul_pd(_mm_shuffle_pd(b.interval, b.interval, 3),
                                      _mm_shuffle_pd(interval, interval, 1)));
  case 2:
    return interval_number(
        _mm_mul_pd(_mm_shuffle_pd(b.interval, b.interval, 0), interval));
  case 4:
    return interval_number(
        _mm_mul_pd(_mm_shuffle_pd(interval, interval, 3),
                   _mm_shuffle_pd(b.interval, b.interval, 1)));
  case 5:
    ip = _mm_mul_pd(_mm_castsi128_pd(_mm_xor_si128(_mm_castpd_si128(interval),
                                                   sign_high_mask)),
                    b.interval);
  ...
  case 6:
    ...
    return interval_number(
        _mm_mul_pd(interval, _mm_shuffle_pd(_mm_castsi128_pd(ssg),
                                            _mm_castsi128_pd(ssg), 1)));
  case 8:
    return interval_number(
        _mm_mul_pd(_mm_shuffle_pd(interval, interval, 0), b.interval));
  case 9:
    ssg = _mm_xor_si128(_mm_castpd_si128(interval), sign_low_mask);
    return interval_number(
        _mm_mul_pd(b.interval, _mm_shuffle_pd(_mm_castsi128_pd(ssg),
                                              _mm_castsi128_pd(ssg), 1)));
  case 10:
    return interval_number(_mm_mul_pd(
        interval, _mm_castsi128_pd(_mm_xor_si128(_mm_castpd_si128(b.interval),
                                                 sign_low_mask))));
  }
...
interval_number::operator*(const interval_number &b) const {
  ...
  case 8:
    return interval_number(high * b.min_low, high * b.high);
  case 9:
    return interval_number(high * b.min_low, (-min_low) * b.high);
  case 2:
    return interval_number(min_low * b.high, high * b.high);
  case 0:
    double ll, lh, hl, hh;
  ...
  case 1:
    return interval_number(high * b.min_low, min_low * b.min_low);
  case 6:
    return interval_number(min_low * b.high, high * (-b.min_low));
  case 4:
    return interval_number(min_low * b.high, min_low * b.min_low);
  case 5:
    return interval_number((-high) * b.high, min_low * b.min_low);
  };
...
inline void expansionObject::Two_Prod(const double a, const double b, double &x,
                                      double &y) {
  x = a * b;
  //			y = fma(a, b, -x);
  Split(a, _ah, _al);
...
inline void expansionObject::Two_One_Prod(const double a1, const double a0,
                                          const double b, double &x3,
...
inline void expansionObject::Two_Two_Prod(const double a1, const double a0,
...
inline int expansionObject::Gen_Sum(const int elen, const double *e,
...
inline int expansionObject::Gen_Diff(const int elen, const double *e,
                                     const int flen, const double *f,
                                     double *h) {
  double Q, Qn, hh, en = e[0], fn = -f[0];
  int e_k, f_k, h_k;
...
inline int expansionObject::Gen_Scale(const int elen, const double *e,
...
inline void expansionObject::Two_Square(const double &a1, const double &a0,
                                        double *x) {
  Square(a0, _j, x[0]);
  _0 = a0 + a0;
...
inline int expansionObject::Sub_product(const int alen, const double *a,
  ...
  double ph1_p[1024];
  double *ph1 =
      (allmem > 1024) ? ((double *)malloc(allmem * sizeof(double))) : (ph1_p);
...
inline int expansionObject::Gen_Product(const int alen, const double *a,
                                        const int blen, const double *b,
                                        double *h) {
  if (blen == 1)
...
inline int expansionObject::Gen_Product_With_Alloc(const int alen,
                                                   const double *a,
                                                   const int blen,
                                                   const double *b,
                                                   double **h) {
...
inline int expansionObject::Double_With_PreAlloc(const int elen,
                                                 const double *e, double **h,
                                                 const int hlen) {
  int newlen = elen;
  if (hlen < newlen)
...
inline int
expansionObject::Gen_Product_With_PreAlloc(const int alen, const double *a,
                                           const int blen, const double *b,
...
inline double bigfloat::get_d() const {
  ...
  if (sign < 0)
    dn |= 0x8000000000000000;           // Set sign
...
inline bigrational bigrational::operator+(const bigrational &r) const {
  if (sign == 0)
    ...
    // if (sign > 0 && r.sign > 0)	return bigrational(left_num + right_num,
    // den3, 1); else if (sign < 0 && r.sign < 0) return bigrational(left_num +
    // right_num, den3, -1); else if (sign > 0 && r.sign < 0) { 	if
    // (left_num
    // >=
    // right_num) return bigrational(left_num - right_num, den3, 1); 	else
    // return bigrational(right_num - left_num, den3, -1);
    // }
    // else { // if (sign < 0 && r.sign > 0)
    //	if (left_num >= right_num) return bigrational(left_num - right_num,
    // den3, -1); 	else return bigrational(right_num - left_num, den3, 1);
    // }
    ...
    if (sign > 0 && r.sign > 0)
      ...
      if (left_num >= right_num)
        return bigrational(left_num - right_num, denominator * r.denominator,
                           1);
      else
        return bigrational(right_num - left_num, denominator * r.denominator,
                           -1);
    } else { // if (sign < 0 && r.sign > 0)
      if (left_num >= right_num)
        return bigrational(left_num - right_num, denominator * r.denominator,
...
inline double bigrational::get_d() const {
  ...
  int32_t E = (int32_t)num.getNumSignificantBits() -
              (int32_t)den.getNumSignificantBits();
  if (E > 0) {
    den <<= E;
    if (den > num) {
      E--;
