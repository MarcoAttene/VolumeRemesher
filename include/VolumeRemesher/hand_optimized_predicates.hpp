    ...
    double sz)
{
    ...
    eb = 7.7715611723761027e-016 * (fabs(fbdxcdy) + fabs(fcdxbdy) + fabs(fcdxady) + fabs(fadxcdy) +
                                    fabs(fadxbdy) + fabs(fbdxady));
    return ((det >= eb) - (-det >= eb));
}

inline int orient3d_bigfloat(
    const bigfloat px,
    const bigfloat py,
    const bigfloat pz,
    const bigfloat qx,
    ...
    const bigfloat sz)
{
    ...
    bigfloat fbdxcdy = fbdx * fcdy;
    bigfloat fcdxbdy = fcdx * fbdy;
    bigfloat fcdxady = fcdx * fady;
    bigfloat fadxcdy = fadx * fcdy;
    bigfloat fadxbdy = fadx * fbdy;
    bigfloat fbdxady = fbdx * fady;

    bigfloat det =
        (fbdxcdy - fcdxbdy) * fadz + (fcdxady - fadxcdy) * fbdz + (fadxbdy - fbdxady) * fcdz;
    return sgn(det);
}

inline int orient3d_exact(
    double pdx,
    double pdy,
    double pdz,
    double pax,
    ...
    double pcz)
{
    ...
    if ((adx[0] == 0.0) && (bdx[0] == 0.0) && (cdx[0] == 0.0) && (ady[0] == 0.0) &&
        (bdy[0] == 0.0) && (cdy[0] == 0.0) && (adz[0] == 0.0) && (bdz[0] == 0.0) && (cdz[0] == 0.0))
        return (det > 0) - (det < 0);
