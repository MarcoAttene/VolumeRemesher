...
inline interval_number operator+(const double a, const interval_number& p)
{
    return p + a;
}
inline interval_number operator-(const double a, const interval_number& p)
...
inline interval_number operator*(const double b, const interval_number& d)
{
    return d * b;
}
...
public:
    ...
    // [x2,x1,x0] = [a1,a0]-[b]		Subtracts an expansion [b] of length one from an expansion
    // [a1,a0] of length two
    static void two_One_Diff(
        const double a1,
        const double a0,
    ...
    // [x3,x2,x1,x0] = [a1,a0]*[b]		Multiplies an expansion [a1,a0] of length two by an
    // expansion [b] of length one
    static void Two_One_Prod(
        const double a1,
        const double a0,
        const double b,
        double& x3,
    ...
    // [x3,x2,x1,x0] = [a1,a0]-[b1,b0]		Calculates the difference between two expansions of
    // length two
    static void Two_Two_Diff(
        const double a1,
        const double a0,
        const double b1,
        const double b0,
    ...
    static void
    Two_Two_Prod(const double a1, const double a0, const double b1, const double b0, double* h);
    ...
    // Same as above, but 'h' is allocated internally. The caller must still call 'free' to release
    // the memory.
    static int Gen_Sum_With_Alloc(
        const int elen,
        const double* e,
        const int flen,
        const double* f,
    ...
    // 'h' must be allocated by the caller with at least elen+flen components.
    static int
    Gen_Diff(const int elen, const double* e, const int flen, const double* f, double* h);

    // Same as above, but 'h' is allocated internally. The caller must still call 'free' to release
    // the memory.
    static int Gen_Diff_With_Alloc(
        const int elen,
        const double* e,
        const int flen,
        const double* f,
    ...
    // 'h' must be allocated by the caller with at least elen components. This is exact up to
    // overflows.
    ...
    static int
    Sub_product(const int alen, const double* a, const int blen, const double* b, double* h);

    // [h] = [a] * [b]
    // 'h' must be allocated by the caller with at least MAX(2*alen*blen, 8) components.
    static int
    Gen_Product(const int alen, const double* a, const int blen, const double* b, double* h);

    // Same as above, but 'h' is allocated internally. The caller must still call 'free' to release
    // the memory.
    static int Gen_Product_With_Alloc(
        const int alen,
        const double* a,
        const int blen,
        const double* b,
    ...
    // The caller must verify whether reallocation took place, and possibly call 'free' to release
    // the memory. When reallocation takes place, *h becomes different from its original value.
    static int Double_With_PreAlloc(const int elen, const double* e, double** h, const int hlen);
    static int Gen_Scale_With_PreAlloc(
        const int elen,
        const double* e,
        const double& b,
        double** h,
        const int hlen);
    static int Gen_Sum_With_PreAlloc(
        const int elen,
        const double* e,
        const int flen,
        const double* f,
        double** h,
        const int hlen);
    static int Gen_Diff_With_PreAlloc(
        const int elen,
        const double* e,
        const int flen,
        const double* f,
        double** h,
        const int hlen);
    static int Gen_Product_With_PreAlloc(
        const int alen,
        const double* a,
        const int blen,
        const double* b,
    ...
    static void Two_Product_2Presplit(
        double a,
        double _ah,
        double _al,
        double b,
...
public:
    ...
    static void initPool(expansionPool* ep)
    {
        pool = ep;
        pool->init();
    }
...
{
    gmp_fscanf(fp, "%Qd,", r.get_mpq_t());
    ungetc(',', fp);
}

inline bigfloat getBigFloatFromRational(const bigrational& r, uint32_t prec_bits)
{
    return r;
}
...
inline bignatural operator+(uint32_t a, const bignatural& p)
{
    return p + a;
}
inline bignatural operator+(uint64_t a, const bignatural& p)
{
    return p + a;
}
inline bignatural operator*(uint64_t a, const bignatural& p)
...
inline bignatural sqrt(const bignatural& n)
{
    return n.sqrt();
}
...
{
    bignatural
        mantissa; // .back() is less significant. Use 32-bit limbs to avoid overflows using 64-bits
...
inline bigfloat operator+(const double a, const bigfloat& p)
{
    return p + a;
}
inline bigfloat operator-(const double a, const bigfloat& p)
...
public:
    ...
    bigrational(const double f)
        : bigrational(bigfloat(f))
    {}
...
// Operators with left-hand doubles
inline bigrational operator+(const double a, const bigrational& p)
{
    return p + a;
}
inline bigrational operator-(const double a, const bigrational& p)
...
inline bigrational operator/(const double a, const bigrational& p)
{
    return bigrational(a) / p;
}

// Sign inversion operator for bigrationals
inline bigrational operator-(const bigrational& p)
{
    return p.negation();
}

// Sign operator for bigrationals
inline int32_t sgn(const bigrational& p)
{
    return p.sgn();
}
...
inline bigfloat getBigFloatFromRational(const bigrational& r, uint32_t prec_bits)
{
    return r.get_bigfloat(prec_bits);
}
...
inline N_memory_pool::N_memory_pool(size_t _size, size_t _block_size)
    : last(_size)
    , size(_size)
...
inline void
IN_pool::addDataBlockArray(uint32_t tot_size, uint32_t extended_block_size, uint32_t** fs)
{
...
inline IN_pool::IN_pool(uint32_t _size, uint32_t _block_size)
    : last(_size)
    , size(_size)
    , block_size(_block_size)
{
...
inline MultiPool::MultiPool(uint32_t _max_block_size, uint32_t init_capacity)
...
#ifdef USE_SIMD_INSTRUCTIONS
inline __m128d interval_number::zero()
{
    return _mm_setzero_pd();
}
inline __m128d interval_number::minus_one()
{
    return _mm_set1_pd(-1.0);
}
inline __m128d interval_number::sign_low_mask()
{
    return _mm_castsi128_pd(_mm_set_epi64x(LLONG_MIN, 0));
}
inline __m128d interval_number::sign_high_mask()
{
    return _mm_castsi128_pd(_mm_set_epi64x(0, LLONG_MIN));
}
inline __m128d interval_number::sign_fabs_mask()
{
    return _mm_castsi128_pd(_mm_set_epi64x(~LLONG_MIN, ~LLONG_MIN));
}
inline __m128d interval_number::all_high_mask()
{
    return _mm_castsi128_pd(_mm_set_epi64x(0, -1LL));
}

inline __m128d interval_number::getLowSwitched() const
{
    return _mm_xor_pd(interval, sign_low_mask());
}

inline interval_number::interval_number() {}
inline interval_number::interval_number(const double a)
    : interval(_mm_set_pd(-a, a))
{}
inline interval_number::interval_number(const double minf, const double sup)
    : interval(_mm_set_pd(minf, sup))
{}
inline interval_number::interval_number(const __m128d& i)
    : interval(i)
{}
inline interval_number::interval_number(const interval_number& b)
    : interval(b.interval)
{}

inline double interval_number::minus_inf() const
{
    return _mm_cvtsd_f64(_mm_shuffle_pd(interval, interval, 1));
}
inline double interval_number::inf() const
{
    return -minus_inf();
}
inline double interval_number::sup() const
{
    return _mm_cvtsd_f64(interval);
}

inline const double* interval_number::getInterval() const
{
    return (const double*)&interval;
}
inline interval_number& interval_number::operator=(const interval_number& b)
{
    interval = b.interval;
    return *this;
}
...
inline interval_number interval_number::operator-() const
{
...
inline interval_number& interval_number::operator+=(const double b)
{
    return operator=(*this + b);
}
inline interval_number& interval_number::operator-=(const double b)
...
inline bool interval_number::operator<(const double b) const
{
    return sup() < b;
}
inline bool interval_number::operator<=(const double b) const
...
inline void interval_number::negate()
{
    interval = _mm_shuffle_pd(interval, interval, 1);
}

inline bool interval_number::isNegative() const
{
    return _mm_comilt_sd(interval, zero());
}
inline bool interval_number::isPositive() const
{
    return _mm_comilt_sd(_mm_shuffle_pd(interval, interval, 1), zero());
}
inline int interval_number::sign() const
{
...
inline interval_number interval_number::fmadd(const interval_number& b, const interval_number& c)
    const
{
    // Fill i1 and i2 with two copies of 'this' and 'b' respectively
    __m256d i1 = _mm256_castpd128_pd256(interval);
...
inline interval_number interval_number::fmsub(const interval_number& b, const interval_number& c)
    const
{
...
inline interval_number interval_number::operator*(const interval_number& b) const
{
    ...
    case 0: // -+ * -+: <min(<a0*b1>,<a1*b0>), max(<a0*b0>,<a1*b1>)>
        ...
        return interval_number(
            _mm_max_pd(_mm_unpacklo_pd(llhh, lhhl), _mm_unpackhi_pd(llhh, lhhl)));
    case 1: // -+ * --: <b0*a1, b0*a0>
        return interval_number(_mm_mul_pd(
            _mm_shuffle_pd(b.interval, b.interval, 3),
            _mm_shuffle_pd(interval, interval, 1)));
    case 2: // -+ * ++: <b1*a0, b1*a1>
        return interval_number(_mm_mul_pd(_mm_shuffle_pd(b.interval, b.interval, 0), interval));
    case 4: // -- * -+: <a0*b1, a0*b0>
        return interval_number(_mm_mul_pd(
            _mm_shuffle_pd(interval, interval, 3),
            _mm_shuffle_pd(b.interval, b.interval, 1)));
    case 5: // -- * --: <a1*b1, a0*b0>
...
inline interval_number interval_number::abs() const
{
    ...
    case 0: // Hi>0, Lo<0
        return _mm_and_pd(
            _mm_max_pd(interval, _mm_shuffle_pd(interval, interval, 1)),
            all_high_mask());
    case 1: // Hi<0, Lo<0
...
inline const double* interval_number::getInterval() const
{
    return (const double*)&min_low;
}

inline interval_number::interval_number() {}
inline interval_number::interval_number(double a)
    : min_low(-a)
    , high(a)
{}
inline interval_number::interval_number(double minf, double sup)
    : min_low(minf)
    , high(sup)
{}
inline interval_number::interval_number(const interval_number& b)
...
inline interval_number interval_number::operator-() const
{
    return interval_number(high, min_low);
}
inline interval_number interval_number::operator+(const double b) const
{
...
inline interval_number interval_number::operator*(const interval_number& b) const
{
    typedef union error_approx_type_t {
        ...
        inline error_approx_type_t(double a)
            : d(a)
        {}
    ...
    uint64_t cfg = (l1.is_negative() << 3) + (h1.is_negative() << 2) + (l2.is_negative() << 1) +
                   (h2.is_negative());
...
inline interval_number interval_number::pow2() const
{
    if (fabs(min_low) > fabs(high)) {
        if (min_low < 0 || high < 0)
            return interval_number((-high) * (high), min_low * min_low);
        else
            return interval_number(0, min_low * min_low);
    } else {
        if (min_low < 0 || high < 0)
            return interval_number((-min_low) * (min_low), high * high);
        else
            return interval_number(0, high * high);
    }
}
...
inline double interval_number::width() const
{
    return sup() - inf();
}

inline bool interval_number::signIsReliable() const
{
    return (isNegative() || isPositive());
} // Zero is not accounted for
inline bool interval_number::containsZero() const
{
    return !signIsReliable();
}

inline bool interval_number::isNAN() const
{
    return sup() != sup();
}

inline double interval_number::getMid() const
{
    return (inf() + sup()) / 2;
}
inline bool interval_number::isExact() const
{
    return inf() == sup();
}

inline bool interval_number::operator<(const interval_number& b) const
{
    return (sup() < b.inf());
}
inline bool interval_number::operator>(const interval_number& b) const
...
inline void expansionObject::Quick_Two_Sum(const double a, const double b, double& x, double& y)
{
    x = a + b;
    y = b - (x - a);
}

inline void expansionObject::quick_Two_Sum(const double a, const double b, double* xy)
{
    Quick_Two_Sum(a, b, xy[1], xy[0]);
}
...
inline void expansionObject::two_Sum(const double a, const double b, double* xy)
{
    Two_Sum(a, b, xy[1], xy[0]);
}

inline void expansionObject::Two_One_Sum(
    const double a1,
    const double a0,
    const double b,
    double& x2,
...
inline void expansionObject::Two_One_Diff(
    const double a1,
    const double a0,
    const double b,
    double& x2,
...
inline void expansionObject::two_Diff(const double a, const double b, double* xy)
{
...
inline void
expansionObject::Two_Prod_PreSplit(double a, double b, double _bh, double _bl, double& x, double& y)
{
    double _ah, _al;
    x = a * b;
    Split(a, _ah, _al);
...
inline void expansionObject::Two_Product_2Presplit(
    double a,
    double _ah,
    double _al,
    double b,
...
// [x,y] = [a]*[b]		 Multiplies two expansions [a] and [b] of length one
inline void expansionObject::Two_Prod(const double a, const double b, double* xy)
{
...
// [x,y] = [a]^2		Squares an expansion of length one
inline void expansionObject::Square(const double a, double* xy)
{
    Square(a, xy[1], xy[0]);
}

// [x2,x1,x0] = [a1,a0]-[b]		Subtracts an expansion [b] of length one from an expansion [a1,a0]
// of length two
inline void expansionObject::two_One_Diff(
    const double a1,
    const double a0,
...
inline void expansionObject::two_One_Diff(const double* a, const double b, double* x)
{
    two_One_Diff(a[1], a[0], b, x[2], x[1], x[0]);
}

// [x3,x2,x1,x0] = [a1,a0]*[b]		Multiplies an expansion [a1,a0] of length two by an expansion
// [b] of length one
inline void expansionObject::Two_One_Prod(const double* a, const double b, double* x)
{
    Two_One_Prod(a[1], a[0], b, x[3], x[2], x[1], x[0]);
}

// [x3,x2,x1,x0] = [a1,a0]+[b1,b0]		Calculates the sum of two expansions of length two
inline void expansionObject::Two_Two_Sum(
    const double a1,
    const double a0,
    const double b1,
    const double b0,
...
inline void expansionObject::Two_Two_Sum(const double* a, const double* b, double* xy)
{
    Two_Two_Sum(a[1], a[0], b[1], b[0], xy[3], xy[2], xy[1], xy[0]);
}

// [x3,x2,x1,x0] = [a1,a0]-[b1,b0]		Calculates the difference between two expansions of length
// two
inline void expansionObject::Two_Two_Diff(
    const double a1,
    const double a0,
    const double b1,
    const double b0,
...
inline void expansionObject::Two_Two_Diff(const double* a, const double* b, double* x)
{
...
inline void expansionObject::Two_Diff_Back(const double a, const double b, double* xy)
{
    Two_Diff_Back(a, b, xy[1], xy[0]);
}

inline void expansionObject::Two_Two_Prod(const double* a, const double* b, double* xy)
{
    Two_Two_Prod(a[1], a[0], b[1], b[0], xy);
}

// [e] = -[e]		Inplace inversion
inline void expansionObject::Gen_Invert(const int elen, double* e)
{
    for (int i = 0; i < elen; i++) e[i] = -e[i];
}

// Same as above, but 'h' is allocated internally. The caller must still call 'free' to release the
// memory.
inline int expansionObject::Gen_Sum_With_Alloc(
    const int elen,
    const double* e,
    const int flen,
    const double* f,
...
// Same as above, but 'h' is allocated internally. The caller must still call 'free' to release the
// memory.
inline int expansionObject::Gen_Diff_With_Alloc(
    const int elen,
    const double* e,
    const int flen,
    const double* f,
...
inline void expansionObject::Double(const int elen, const double* e, double* h)
{
...
// [h] = [e] * n		Multiplies an expansion by n
// If 'n' is a power of two, the multiplication is exact
inline void expansionObject::ExactScale(const int elen, double* e, const double n)
{
    for (int i = 0; i < elen; i++) e[i] *= n;
}

inline void expansionObject::print(const int elen, const double* e)
{
...
inline void expansionObject::Two_One_Prod(
    const double a1,
    const double a0,
    const double b,
    double& x3,
...
inline void expansionObject::Two_Two_Prod(
    const double a1,
    const double a0,
    const double b1,
    const double b0,
...
inline int expansionObject::Gen_Sum(
    const int elen,
    const double* e,
    const int flen,
    const double* f,
...
inline int expansionObject::Gen_Diff(
    const int elen,
    const double* e,
    const int flen,
    const double* f,
...
inline void expansionObject::Two_Square(const double* a, double* x)
{
    Two_Square(a[1], a[0], x);
}

inline int expansionObject::Sub_product(
    const int alen,
    const double* a,
    const int blen,
    const double* b,
...
inline int expansionObject::Gen_Product(
    const int alen,
    const double* a,
    const int blen,
    const double* b,
...
inline int expansionObject::Gen_Product_With_Alloc(
    const int alen,
    const double* a,
    const int blen,
    const double* b,
...
inline int
expansionObject::Double_With_PreAlloc(const int elen, const double* e, double** h, const int hlen)
{
    int newlen = elen;
...
inline int expansionObject::Gen_Scale_With_PreAlloc(
    const int elen,
    const double* e,
    const double& b,
    double** h,
...
inline int expansionObject::Gen_Sum_With_PreAlloc(
    const int elen,
    const double* e,
    const int flen,
    const double* f,
...
inline int expansionObject::Gen_Diff_With_PreAlloc(
    const int elen,
    const double* e,
    const int flen,
    const double* f,
...
inline int expansionObject::Gen_Product_With_PreAlloc(
    const int alen,
    const double* a,
    const int blen,
    const double* b,
...
inline expansion::expansion(size_t s)
    : els(pool->next)
    , len(s)
{
...
inline uint32_t* bignatural::BN_ALLOC(uint32_t num_bytes)
{
    return (uint32_t*)nfgMemoryPool.alloc(num_bytes);
}
inline void bignatural::BN_FREE(uint32_t* ptr)
{
    nfgMemoryPool.release(ptr);
}
inline bignatural::bignatural()
    : m_capacity(0)
    , m_size(0)
    , digits(NULL)
{}
...
inline bignatural::bignatural(const bignatural& m)
{
    init(m);
}
inline bignatural::bignatural(bignatural&& m) noexcept
...
inline bignatural::bignatural(uint32_t m)
{
    init(m);
}

inline bignatural::bignatural(uint64_t m)
{
    init(m);
}

inline bignatural::bignatural(FILE* f)
{
    init(f);
}

inline const uint32_t& bignatural::back() const
{
    return digits[m_size - 1];
}

inline const uint32_t& bignatural::operator[](int i) const
{
    return digits[i];
}

inline uint32_t bignatural::size() const
{
    return m_size;
}

inline bool bignatural::empty() const
{
    return m_size == 0;
}

inline bignatural bignatural::operator<<(uint32_t n) const
{
    bignatural t(*this);
    t <<= n;
    return t;
...
inline bignatural bignatural::operator+(const uint32_t b) const
{
    bignatural n(*this);
    return (n += b);
}
inline bignatural bignatural::operator+(const uint64_t b) const
{
    return operator+(bignatural(b));
}
...
inline void bignatural::operator|=(uint32_t i)
{
    if (m_size)
        digits[m_size - 1] |= i;
    else
...
inline uint32_t& bignatural::back()
{
    return digits[m_size - 1];
}

inline void bignatural::pop_back()
{
    m_size--;
}

inline uint32_t& bignatural::operator[](int i)
{
    return digits[i];
}

inline void bignatural::reserve(uint32_t n)
{
    if (n > m_capacity) increaseCapacity(n);
}

inline void bignatural::resize(uint32_t n)
{
    reserve(n);
    m_size = n;
}
...
inline bigfloat::bigfloat(const bignatural& m, int32_t e, int32_t s)
    : mantissa(m)
    , exponent(e)
    , sign(s)
{}

inline int32_t bigfloat::log2() const
{
    return exponent + ((int32_t)mantissa.getNumSignificantBits()) - 1;
}

inline void bigfloat::increaseMantissa()
{
    mantissa += 1U;
    pack();
}
...
inline bool bigfloat::operator<(const bigfloat& b) const
{
    ...
    if (sign >= 0) {
        if (na < nb)
            ...
            return mantissa < (b.mantissa << (b.exponent - exponent));
        else if (exponent > b.exponent)
            return (mantissa << (exponent - b.exponent)) < b.mantissa;
        else
            return mantissa < b.mantissa;
    } else {
        if (na < nb)
            return false;
        else if (na > nb)
...
inline void bigfloat::invert()
{
    sign = -sign;
}

inline int bigfloat::sgn() const
{
    return sign;
}

inline const bignatural& bigfloat::getMantissa() const
{
    return mantissa;
}

inline int32_t bigfloat::getExponent() const
{
    return exponent;
}
...
inline bigrational bigrational::inverse() const
{
    bigrational r = *this;
    r.invert();
    return r;
...
inline void bigrational::negate()
{
    sign = -sign;
}

inline bigrational bigrational::negation() const
{
    bigrational r = *this;
    r.negate();
    return r;
...
{
    return (
        sign > r.sign || (sign > 0 && r.sign > 0 && hasGreaterModule(r)) ||
...
{
    return (
        sign > r.sign || (sign > 0 && r.sign > 0 && hasGrtrOrEqModule(r)) ||
...
inline const bignatural& bigrational::get_num() const
{
    return numerator;
}
inline const bignatural& bigrational::get_den() const
{
    return denominator;
}

inline int32_t bigrational::sgn() const
{
    return sign;
}
...
inline bigrational::bigrational(const bigfloat& f)
{
    if (f.sgn() == 0)
        ...
        if (e >= 0)
            numerator.leftShift((uint32_t)e);
        else
            denominator.leftShift((uint32_t)(-e));
    }
}
...
inline double bigrational::get_d() const
{
    ...
    if (E > 0) {
        den.leftShift((uint32_t)E);
        if (den > num) {
            E--;
            den >>= 1;
        }
    } else if (E <= 0) {
        num.leftShift((uint32_t)(-E));
        if (den > num) {
            E--;
            num.leftShift(1);
        }
