...
public:
    genericPoint(const Point_Type& t)
        : type(t)
    {}
    ...
    // Sign of (a-c) dot (b-c)
    static int
    dotProductSign2D(const genericPoint& a, const genericPoint& b, const genericPoint& c);
    static int
    dotProductSign3D(const genericPoint& a, const genericPoint& b, const genericPoint& c);
    ...
    // 2 - if m == |z|
    //
    // Warning: this function assumes that the triangle is not exactly degenerate. It may crash
    // otherwise.
    ...
    static bool
    pointInInnerSegment(const genericPoint& p, const genericPoint& v1, const genericPoint& v2);

    // TRUE if 'p' is in the closure of v1-v2
    static bool
    pointInSegment(const genericPoint& p, const genericPoint& v1, const genericPoint& v2);

    // TRUE if P is in the interior of <A,B,C>
    // Points are assumed to be coplanar. Undetermined otherwise.
    static bool pointInInnerTriangle(
    ...
    static bool pointInTriangle(
        ...
        const genericPoint& B,
        const genericPoint& C);
    // Same as above, but this version initializes oAB, oAC and oCA with the orientation of P wrt
    // one of the edges (0 = on edge)
    static bool pointInTriangle(
        const genericPoint& P,
        const genericPoint& A,
        const genericPoint& B,
    ...
    // Points are assumed to be coplanar. Undetermined otherwise.
    static bool innerSegmentsCross(
        const genericPoint& A,
        const genericPoint& B,
        const genericPoint& P,
        const genericPoint& Q);
    ...
    static bool innerSegmentCrossesInnerTriangle(
    ...
    // TRUE if the infinite straight line by s1-s2 intersects triangle <v1,v2,v3> at a single
    // internal point
    static bool lineCrossesInnerTriangle(
        const genericPoint& s1,
        const genericPoint& s2,
        const genericPoint& v1,
        const genericPoint& v2,
    ...
    static bool lineCrossesTriangle(
        const genericPoint& s1,
        const genericPoint& s2,
        const genericPoint& v1,
        const genericPoint& v2,
    ...
    static bool innerSegmentCrossesTriangle(
        const genericPoint& s1,
        const genericPoint& s2,
        const genericPoint& v1,
        const genericPoint& v2,
    ...
    static int
    orient2D(const genericPoint& a, const genericPoint& b, const genericPoint& c, int n_max)
    {
        if (n_max == 0)
    ...
    static bool
    misaligned(const genericPoint& A, const genericPoint& B, const genericPoint& C, int n_max)
    {
        return (
            (n_max == 2 && orient2Dxy(A, B, C)) || (n_max == 0 && orient2Dyz(A, B, C)) ||
            (n_max == 1 && orient2Dzx(A, B, C)));
    }

    static bool pointInInnerSegment(
        const genericPoint& p,
        const genericPoint& v1,
        const genericPoint& v2,
        int n_max);
    static bool pointInSegment(
        const genericPoint& p,
        const genericPoint& v1,
        const genericPoint& v2,
        int n_max);
    ...
    bool getLambda2D(interval_number& lx, interval_number& ly, interval_number& d) const
    {
        return getIntervalLambda(lx, ly, d);
    }
    bool getLambda3D(
        interval_number& lx,
        interval_number& ly,
        ...
        interval_number& d) const
    {
        return getIntervalLambda(lx, ly, lz, d);
    }
    bool getLambda2D(bigfloat& lx, bigfloat& ly, bigfloat& d) const
    {
        getBigfloatLambda(lx, ly, d);
        return true;
    }
    bool getLambda3D(bigfloat& lx, bigfloat& ly, bigfloat& lz, bigfloat& d) const
    {
        getBigfloatLambda(lx, ly, lz, d);
        return true;
    }
    bool getLambda2D(expansion& lx, expansion& ly, expansion& d) const
    {
        getExpansionLambda(lx, ly, d);
        return true;
    }
    bool getLambda3D(expansion& lx, expansion& ly, expansion& lz, expansion& d) const
    {
        getExpansionLambda(lx, ly, lz, d);
        return true;
    }
};
...
public:
    explicitPoint2D()
        : genericPoint(Point_Type::EXPLICIT2D)
    {}
...
public:
    explicitPoint3D()
        : genericPoint(Point_Type::EXPLICIT3D)
    {}
...
public:
    implicitPoint3D_LNC(const genericPoint& _p, const genericPoint& _q, const double _t);
