...
namespace vol_rem {
static inline double ipow2(const double& d)
{
    return d * d;
}
static inline double ipow3(const double& d)
{
    return d * ipow2(d);
}

static inline interval_number ipow2(const interval_number& d)
{
    return d.pow2();
}
static inline interval_number ipow3(const interval_number& d)
{
    return d.pow3();
}

static inline expansion ipow2(const expansion& d)
{
    return d.sqr();
}
static inline expansion ipow3(const expansion& d)
{
    return d.sqr() * d;
}

static inline bigfloat ipow2(const bigfloat& d)
{
    return d * d;
}
static inline bigfloat ipow3(const bigfloat& d)
{
    return d * ipow2(d);
}

inline int sgn(const interval_number& p)
{
...
static inline int dotProductSign2D_t(
    const PT& px,
    const PT& py,
    const PT& rx,
    const PT& ry,
    ...
    const PT& qy)
{
    ...
    if constexpr (std::is_same<double, T>::value) {
        ...
        _tmp_fabs = fabs(lx);
        max_var = (_tmp_fabs > max_var) ? (_tmp_fabs) : (max_var);
        _tmp_fabs = fabs(ly);
        max_var = (_tmp_fabs > max_var) ? (_tmp_fabs) : (max_var);
        _tmp_fabs = fabs(gx);
        max_var = (_tmp_fabs > max_var) ? (_tmp_fabs) : (max_var);
    ...
    const double& qy)
{
    ...
    if ((ret = dotProductSign2D_t<s_expansion, expansion>(px, py, rx, ry, qx, qy)) != INT_MAX)
        return ret;
...
template <class PT, class T>
static inline int dotProductSign3D_t(
    const PT& px,
    const PT& py,
    const PT& pz,
    const PT& rx,
...
{
    int ret;
    if ((ret = dotProductSign3D_t<double, double>(px, py, pz, rx, ry, rz, qx, qy, qz)) != 0)
        return ret;
    if ((ret = dotProductSign3D_t<s_expansion, expansion>(px, py, pz, rx, ry, rz, qx, qy, qz)) !=
        INT_MAX)
        return ret;
    ...
    const double& pdy)
{
    ...
    if ((ret = incircle_t<s_expansion, expansion>(pax, pay, pbx, pby, pcx, pcy, pdx, pdy)) !=
        INT_MAX)
        return ret;
...
template <class PT, class T>
static inline int inGabrielSphere_t(
    const PT& qx,
    const PT& qy,
    const PT& qz,
    const PT& ax,
    ...
    const PT& cz)
{
    ...
    const T ret =
        ((qax * ((qax * c2) - ccax)) + ((qay * ((qay * c2) - ccay)) + (qaz * ((qaz * c2) - ccaz))));
...
{
    int ret;
    if ((ret = inGabrielSphere_t<double, double>(qx, qy, qz, ax, ay, az, bx, by, bz, cx, cy, cz)) !=
        0)
        return ret;
    ...
    const double& pez)
{
    ...
    if ((ret = inSphere_t<
             double,
             double>(pax, pay, paz, pbx, pby, pbz, pcx, pcy, pcz, pdx, pdy, pdz, pex, pey, pez)) !=
        0)
        return ret;
    return inSphere_t<bigfloat, bigfloat>(
        pax,
        pay,
        paz,
        pbx,
...
template <class PT, class T>
static inline int dotProductSign2D_EEI_t(
...
inline int dotProductSign2D_EEI(
    const genericPoint& q,
    const double& px,
    const double& py,
    const double& rx,
...
{
    int ret;
    if ((ret = dotProductSign2D_EEI_t<interval_number, interval_number>(q, px, py, rx, ry)) != 0)
        return ret;
...
template <class PT, class T>
static inline int dotProductSign2D_IEE_t(
...
inline int dotProductSign2D_IEE(
    const genericPoint& p,
    const double& rx,
    const double& ry,
    const double& qx,
...
{
    int ret;
    if ((ret = dotProductSign2D_IEE_t<interval_number, interval_number>(p, rx, ry, qx, qy)) != 0)
        return ret;
...
template <class PT, class T>
static inline int
dotProductSign2D_IEI_t(const genericPoint& p, const genericPoint& q, const PT& rx, const PT& ry)
{
    if constexpr (std::is_same<interval_number, T>::value) setFPUModeToRoundUP();
...
inline int dotProductSign2D_IEI(
    const genericPoint& p,
    const genericPoint& q,
    const double& rx,
    const double& ry)
{
    int ret;
    if ((ret = dotProductSign2D_IEI_t<interval_number, interval_number>(p, q, rx, ry)) != 0)
        return ret;
...
template <class PT, class T>
static inline int
dotProductSign2D_IIE_t(const genericPoint& p, const genericPoint& r, const PT& qx, const PT& qy)
{
    if constexpr (std::is_same<interval_number, T>::value) setFPUModeToRoundUP();
...
inline int dotProductSign2D_IIE(
    const genericPoint& p,
...
{
    int ret;
    if ((ret = dotProductSign2D_IIE_t<interval_number, interval_number>(p, r, qx, qy)) != 0)
        return ret;
...
template <class PT, class T>
static inline int
dotProductSign2D_III_t(const genericPoint& p, const genericPoint& r, const genericPoint& q)
{
    if constexpr (std::is_same<interval_number, T>::value) setFPUModeToRoundUP();
...
template <class PT, class T>
static inline int dotProductSign3D_EEI_t(
    const genericPoint& q,
    const PT& px,
    const PT& py,
    const PT& pz,
...
inline int dotProductSign3D_EEI(
    const genericPoint& q,
    const double& px,
    const double& py,
    const double& pz,
    ...
    const double& rz)
{
    int ret;
    if ((ret =
             dotProductSign3D_EEI_t<interval_number, interval_number>(q, px, py, pz, rx, ry, rz)) !=
        0)
        return ret;
...
template <class PT, class T>
static inline int dotProductSign3D_IEE_t(
    const genericPoint& p,
    const PT& rx,
    const PT& ry,
    const PT& rz,
...
inline int dotProductSign3D_IEE(
    const genericPoint& p,
    const double& rx,
    const double& ry,
    const double& rz,
    ...
    const double& qz)
{
    int ret;
    if ((ret =
             dotProductSign3D_IEE_t<interval_number, interval_number>(p, rx, ry, rz, qx, qy, qz)) !=
        0)
        return ret;
...
template <class PT, class T>
static inline int dotProductSign3D_IEI_t(
    const genericPoint& p,
    const genericPoint& q,
    const PT& rx,
    const PT& ry,
...
inline int dotProductSign3D_IEI(
    const genericPoint& p,
    const genericPoint& q,
    const double& rx,
    const double& ry,
...
{
    int ret;
    if ((ret = dotProductSign3D_IEI_t<interval_number, interval_number>(p, q, rx, ry, rz)) != 0)
        return ret;
...
template <class PT, class T>
static inline int dotProductSign3D_IIE_t(
    const genericPoint& p,
    const genericPoint& r,
    const PT& qx,
    const PT& qy,
...
inline int dotProductSign3D_IIE(
    const genericPoint& p,
    const genericPoint& r,
    const double& qx,
    const double& qy,
...
{
    int ret;
    if ((ret = dotProductSign3D_IIE_t<interval_number, interval_number>(p, r, qx, qy, qz)) != 0)
        return ret;
...
template <class PT, class T>
static inline int
dotProductSign3D_III_t(const genericPoint& p, const genericPoint& r, const genericPoint& q)
{
    if constexpr (std::is_same<interval_number, T>::value) setFPUModeToRoundUP();
...
template <class PT, class T>
static inline int incirclexy_indirect_IEEE_t(
...
inline int incirclexy_indirect_IEEE(
    const genericPoint& p1,
    const double& pbx,
    const double& pby,
    const double& pcx,
...
{
    int ret;
    if ((ret = incirclexy_indirect_IEEE_t<interval_number, interval_number>(
             p1,
             pbx,
             pby,
             pcx,
...
template <class PT, class T>
static inline int incirclexy_indirect_IIEE_t(
    const genericPoint& p1,
...
inline int incirclexy_indirect_IIEE(
    const genericPoint& p1,
    const genericPoint& p2,
    const double& pcx,
    const double& pcy,
...
{
    int ret;
    if ((ret = incirclexy_indirect_IIEE_t<interval_number, interval_number>(
             p1,
             p2,
             pcx,
             pcy,
...
template <class PT, class T>
static inline int incirclexy_indirect_IIIE_t(
    const genericPoint& p1,
    const genericPoint& p2,
...
inline int incirclexy_indirect_IIIE(
    const genericPoint& p1,
    const genericPoint& p2,
    const genericPoint& p3,
    const double& pdx,
    const double& pdy)
{
    int ret;
    if ((ret =
             incirclexy_indirect_IIIE_t<interval_number, interval_number>(p1, p2, p3, pdx, pdy)) !=
        0)
        return ret;
...
template <class PT, class T>
static inline int incirclexy_indirect_IIII_t(
    const genericPoint& p1,
    const genericPoint& p2,
    const genericPoint& p3,
...
inline int incirclexy_indirect_IIII(
    const genericPoint& p1,
    const genericPoint& p2,
    const genericPoint& p3,
    const genericPoint& p4)
{
    int ret;
    if ((ret = incirclexy_indirect_IIII_t<interval_number, interval_number>(p1, p2, p3, p4)) != 0)
        return ret;
...
template <class PT, class T>
static inline int incircle_indirect_IEEE_t(
    const genericPoint& p1,
    const PT& pbx,
    const PT& pby,
    const PT& pcx,
...
inline int incircle_indirect_IEEE(
    const genericPoint& p1,
    const double& pbx,
    const double& pby,
    const double& pcx,
...
{
    int ret;
    if ((ret = incircle_indirect_IEEE_t<interval_number, interval_number>(
             p1,
             pbx,
             pby,
             pcx,
...
template <class PT, class T>
static inline int incircle_indirect_IIEE_t(
    const genericPoint& p1,
    const genericPoint& p2,
    const PT& pcx,
    const PT& pcy,
...
inline int incircle_indirect_IIEE(
    const genericPoint& p1,
    const genericPoint& p2,
    const double& pcx,
    const double& pcy,
...
{
    int ret;
    if ((ret = incircle_indirect_IIEE_t<interval_number, interval_number>(
             p1,
             p2,
             pcx,
             pcy,
...
template <class PT, class T>
static inline int incircle_indirect_IIIE_t(
    const genericPoint& p1,
    const genericPoint& p2,
    const genericPoint& p3,
    const PT& pdx,
...
inline int incircle_indirect_IIIE(
    const genericPoint& p1,
    const genericPoint& p2,
    const genericPoint& p3,
    const double& pdx,
...
{
    int ret;
    if ((ret = incircle_indirect_IIIE_t<interval_number, interval_number>(p1, p2, p3, pdx, pdy)) !=
        0)
        return ret;
...
template <class PT, class T>
static inline int incircle_indirect_IIII_t(
    const genericPoint& p1,
    const genericPoint& p2,
    const genericPoint& p3,
    const genericPoint& p4)
...
inline int incircle_indirect_IIII(
    const genericPoint& p1,
    const genericPoint& p2,
    const genericPoint& p3,
    const genericPoint& p4)
{
    int ret;
    if ((ret = incircle_indirect_IIII_t<interval_number, interval_number>(p1, p2, p3, p4)) != 0)
        return ret;
...
template <class PT, class T>
static inline int inGabrielSphere_EIEE_t(
...
inline int inGabrielSphere_EIEE(
    const genericPoint& a,
    const double& qx,
    const double& qy,
    const double& qz,
...
{
    int ret;
    if ((ret = inGabrielSphere_EIEE_t<
             interval_number,
             interval_number>(a, qx, qy, qz, bx, by, bz, cx, cy, cz)) != 0)
        return ret;
...
template <class PT, class T>
static inline int inGabrielSphere_EIIE_t(
    const genericPoint& a,
...
inline int inGabrielSphere_EIIE(
    const genericPoint& a,
    const genericPoint& b,
    const double& qx,
    const double& qy,
...
{
    int ret;
    if ((ret = inGabrielSphere_EIIE_t<interval_number, interval_number>(
             a,
             b,
             qx,
             qy,
...
template <class PT, class T>
static inline int inGabrielSphere_EIII_t(
    const genericPoint& a,
    const genericPoint& b,
...
inline int inGabrielSphere_EIII(
    const genericPoint& a,
    const genericPoint& b,
    const genericPoint& c,
    const double& qx,
...
{
    int ret;
    if ((ret = inGabrielSphere_EIII_t<interval_number, interval_number>(a, b, c, qx, qy, qz)) != 0)
        return ret;
...
template <class PT, class T>
static inline int inGabrielSphere_IEEE_t(
    const genericPoint& q,
    const PT& ax,
    const PT& ay,
    const PT& az,
    ...
    const PT& cz)
{
    ...
    const T ret =
        ((qax * ((qax * c2) - (ccax * d1))) +
         ((qay * ((qay * c2) - (ccay * d1))) + (qaz * ((qaz * c2) - (ccaz * d1)))));
...
inline int inGabrielSphere_IEEE(
    const genericPoint& q,
    const double& ax,
    const double& ay,
    const double& az,
...
{
    int ret;
    if ((ret = inGabrielSphere_IEEE_t<
             interval_number,
             interval_number>(q, ax, ay, az, bx, by, bz, cx, cy, cz)) != 0)
        return ret;
...
template <class PT, class T>
static inline int inGabrielSphere_IIEE_t(
    const genericPoint& q,
    const genericPoint& a,
    const PT& bx,
    const PT& by,
...
inline int inGabrielSphere_IIEE(
    const genericPoint& q,
    const genericPoint& a,
    const double& bx,
    const double& by,
...
{
    int ret;
    if ((ret = inGabrielSphere_IIEE_t<interval_number, interval_number>(
             q,
             a,
             bx,
             by,
...
template <class PT, class T>
static inline int inGabrielSphere_IIIE_t(
    const genericPoint& q,
    const genericPoint& a,
    const genericPoint& b,
    const PT& cx,
...
inline int inGabrielSphere_IIIE(
    const genericPoint& q,
    const genericPoint& a,
    const genericPoint& b,
    const double& cx,
...
{
    int ret;
    if ((ret = inGabrielSphere_IIIE_t<interval_number, interval_number>(q, a, b, cx, cy, cz)) != 0)
        return ret;
...
template <class PT, class T>
static inline int inGabrielSphere_IIII_t(
    const genericPoint& q,
    const genericPoint& a,
    const genericPoint& b,
    const genericPoint& c)
...
inline int inGabrielSphere_IIII(
    const genericPoint& q,
    const genericPoint& a,
    const genericPoint& b,
    const genericPoint& c)
{
    int ret;
    if ((ret = inGabrielSphere_IIII_t<interval_number, interval_number>(q, a, b, c)) != 0)
        return ret;
...
template <class PT, class T>
static inline int inSphere_IEEEE_t(
    const genericPoint& p1,
    const PT& pbx,
    const PT& pby,
    const PT& pbz,
...
{
    int ret;
    if ((ret = inSphere_IEEEE_t<
             interval_number,
             interval_number>(p1, pbx, pby, pbz, pcx, pcy, pcz, pdx, pdy, pdz, pex, pey, pez)) != 0)
        return ret;
    return inSphere_IEEEE_t<bigfloat, bigfloat>(
        p1,
        pbx,
        pby,
        pbz,
...
template <class PT, class T>
static inline int inSphere_IIEEE_t(
    const genericPoint& p1,
    const genericPoint& p2,
    const PT& pcx,
    const PT& pcy,
...
{
    int ret;
    if ((ret = inSphere_IIEEE_t<
             interval_number,
             interval_number>(p1, p2, pcx, pcy, pcz, pdx, pdy, pdz, pex, pey, pez)) != 0)
        return ret;
    return inSphere_IIEEE_t<bigfloat, bigfloat>(
        p1,
        p2,
        pcx,
        pcy,
...
template <class PT, class T>
static inline int inSphere_IIIEE_t(
    const genericPoint& p1,
    const genericPoint& p2,
    const genericPoint& p3,
    const PT& pdx,
...
{
    int ret;
    if ((ret = inSphere_IIIEE_t<
             interval_number,
             interval_number>(p1, p2, p3, pdx, pdy, pdz, pex, pey, pez)) != 0)
        return ret;
...
template <class PT, class T>
static inline int inSphere_IIIIE_t(
    const genericPoint& p1,
    const genericPoint& p2,
    const genericPoint& p3,
    const genericPoint& p4,
...
{
    int ret;
    if ((ret = inSphere_IIIIE_t<interval_number, interval_number>(p1, p2, p3, p4, pex, pey, pez)) !=
        0)
        return ret;
...
template <class PT, class T>
static inline int inSphere_IIIII_t(
    const genericPoint& p1,
    const genericPoint& p2,
    const genericPoint& p3,
    const genericPoint& p4,
...
{
    int ret;
    if ((ret = inSphere_IIIII_t<interval_number, interval_number>(p1, p2, p3, p4, p5)) != 0)
        return ret;
...
template <class PT, class T>
static inline int lessThanOnX_IE_t(const genericPoint& p1, const PT& bx)
{
    if constexpr (std::is_same<interval_number, T>::value) setFPUModeToRoundUP();
...
template <class PT, class T>
static inline int lessThanOnX_II_t(const genericPoint& p1, const genericPoint& p2)
{
    if constexpr (std::is_same<interval_number, T>::value) setFPUModeToRoundUP();
...
template <class PT, class T>
static inline int lessThanOnY_IE_t(const genericPoint& p1, const PT& by)
{
    if constexpr (std::is_same<interval_number, T>::value) setFPUModeToRoundUP();
...
template <class PT, class T>
static inline int lessThanOnY_II_t(const genericPoint& p1, const genericPoint& p2)
{
    if constexpr (std::is_same<interval_number, T>::value) setFPUModeToRoundUP();
...
template <class PT, class T>
static inline int lessThanOnZ_IE_t(const genericPoint& p1, const PT& bz)
{
    if constexpr (std::is_same<interval_number, T>::value) setFPUModeToRoundUP();
...
template <class PT, class T>
static inline int lessThanOnZ_II_t(const genericPoint& p1, const genericPoint& p2)
{
    if constexpr (std::is_same<interval_number, T>::value) setFPUModeToRoundUP();
...
template <class PT, class T>
static inline int orient2dxy_indirect_IEE_t(
    const genericPoint& p1,
    const PT& p2x,
    const PT& p2y,
    const PT& p3x,
...
inline int orient2dxy_indirect_IEE(
    const genericPoint& p1,
    const double& p2x,
    const double& p2y,
    const double& p3x,
    const double& p3y)
{
    int ret;
    if ((ret =
             orient2dxy_indirect_IEE_t<interval_number, interval_number>(p1, p2x, p2y, p3x, p3y)) !=
        0)
        return ret;
...
template <class PT, class T>
static inline int orient2dxy_indirect_IIE_t(
    const genericPoint& p1,
    const genericPoint& p2,
    const PT& op3x,
    const PT& op3y)
...
inline int orient2dxy_indirect_IIE(
    const genericPoint& p1,
    const genericPoint& p2,
    const double& op3x,
    const double& op3y)
{
    int ret;
    if ((ret = orient2dxy_indirect_IIE_t<interval_number, interval_number>(p1, p2, op3x, op3y)) !=
        0)
        return ret;
...
template <class PT, class T>
static inline int
orient2dxy_indirect_III_t(const genericPoint& p1, const genericPoint& p2, const genericPoint& p3)
{
    if constexpr (std::is_same<interval_number, T>::value) setFPUModeToRoundUP();
...
inline int
orient2dxy_indirect_III(const genericPoint& p1, const genericPoint& p2, const genericPoint& p3)
{
    int ret;
    if ((ret = orient2dxy_indirect_III_t<interval_number, interval_number>(p1, p2, p3)) != 0)
        return ret;
...
template <class PT, class T>
static inline int orient2dyz_indirect_IEE_t(
    const genericPoint& p1,
    const PT& p2x,
    const PT& p2y,
    const PT& p3x,
...
inline int orient2dyz_indirect_IEE(
    const genericPoint& p1,
    const double& p2x,
    const double& p2y,
    const double& p3x,
    const double& p3y)
{
    int ret;
    if ((ret =
             orient2dyz_indirect_IEE_t<interval_number, interval_number>(p1, p2x, p2y, p3x, p3y)) !=
        0)
        return ret;
...
template <class PT, class T>
static inline int orient2dyz_indirect_IIE_t(
    const genericPoint& p1,
    const genericPoint& p2,
    const PT& op3x,
    const PT& op3y)
...
inline int orient2dyz_indirect_IIE(
    const genericPoint& p1,
    const genericPoint& p2,
    const double& op3x,
    const double& op3y)
{
    int ret;
    if ((ret = orient2dyz_indirect_IIE_t<interval_number, interval_number>(p1, p2, op3x, op3y)) !=
        0)
        return ret;
...
template <class PT, class T>
static inline int
orient2dyz_indirect_III_t(const genericPoint& p1, const genericPoint& p2, const genericPoint& p3)
{
    if constexpr (std::is_same<interval_number, T>::value) setFPUModeToRoundUP();
...
inline int
orient2dyz_indirect_III(const genericPoint& p1, const genericPoint& p2, const genericPoint& p3)
{
    int ret;
    if ((ret = orient2dyz_indirect_III_t<interval_number, interval_number>(p1, p2, p3)) != 0)
        return ret;
...
template <class PT, class T>
static inline int orient2dzx_indirect_IEE_t(
    const genericPoint& p1,
    const PT& p2x,
    const PT& p2y,
    const PT& p3x,
...
inline int orient2dzx_indirect_IEE(
    const genericPoint& p1,
    const double& p2x,
    const double& p2y,
    const double& p3x,
    const double& p3y)
{
    int ret;
    if ((ret =
             orient2dzx_indirect_IEE_t<interval_number, interval_number>(p1, p2x, p2y, p3x, p3y)) !=
        0)
        return ret;
...
template <class PT, class T>
static inline int orient2dzx_indirect_IIE_t(
    const genericPoint& p1,
    const genericPoint& p2,
    const PT& op3x,
    const PT& op3y)
...
inline int orient2dzx_indirect_IIE(
    const genericPoint& p1,
    const genericPoint& p2,
    const double& op3x,
    const double& op3y)
{
    int ret;
    if ((ret = orient2dzx_indirect_IIE_t<interval_number, interval_number>(p1, p2, op3x, op3y)) !=
        0)
        return ret;
...
template <class PT, class T>
static inline int
orient2dzx_indirect_III_t(const genericPoint& p1, const genericPoint& p2, const genericPoint& p3)
{
    if constexpr (std::is_same<interval_number, T>::value) setFPUModeToRoundUP();
...
inline int
orient2dzx_indirect_III(const genericPoint& p1, const genericPoint& p2, const genericPoint& p3)
{
    int ret;
    if ((ret = orient2dzx_indirect_III_t<interval_number, interval_number>(p1, p2, p3)) != 0)
        return ret;
...
template <class PT, class T>
static inline int orient2d_indirect_IEE_t(
    const genericPoint& p1,
    const PT& p2x,
    const PT& p2y,
    const PT& p3x,
...
inline int orient2d_indirect_IEE(
    const genericPoint& p1,
    const double& p2x,
    const double& p2y,
    const double& p3x,
...
{
    int ret;
    if ((ret = orient2d_indirect_IEE_t<interval_number, interval_number>(p1, p2x, p2y, p3x, p3y)) !=
        0)
        return ret;
...
template <class PT, class T>
static inline int orient2d_indirect_IIE_t(
    const genericPoint& p1,
    const genericPoint& p2,
    const PT& p3x,
    const PT& p3y)
...
inline int orient2d_indirect_IIE(
    const genericPoint& p1,
    const genericPoint& p2,
    const double& p3x,
    const double& p3y)
{
    int ret;
    if ((ret = orient2d_indirect_IIE_t<interval_number, interval_number>(p1, p2, p3x, p3y)) != 0)
        return ret;
...
template <class PT, class T>
static inline int
orient2d_indirect_III_t(const genericPoint& p1, const genericPoint& p2, const genericPoint& p3)
{
    if constexpr (std::is_same<interval_number, T>::value) setFPUModeToRoundUP();
...
inline int
orient2d_indirect_III(const genericPoint& p1, const genericPoint& p2, const genericPoint& p3)
{
    int ret;
    if ((ret = orient2d_indirect_III_t<interval_number, interval_number>(p1, p2, p3)) != 0)
        return ret;
...
template <class PT, class T>
static inline int orient3d_indirect_IEEE_t(
    const genericPoint& p1,
    const PT& p2x,
    const PT& p2y,
    const PT& p2z,
    ...
    const PT& p4z)
{
    ...
    const T det =
        (((((v3x * v1z) - (v3z * v1x)) * v2y) - (((v3x * v1y) - (v3y * v1x)) * v2z)) -
         (((v3y * v1z) - (v3z * v1y)) * v2x));
...
inline int orient3d_indirect_IEEE(
    const genericPoint& p1,
    const double& p2x,
    const double& p2y,
    const double& p2z,
...
{
    int ret;
    if ((ret = orient3d_indirect_IEEE_t<
             interval_number,
             interval_number>(p1, p2x, p2y, p2z, p3x, p3y, p3z, p4x, p4y, p4z)) != 0)
        return ret;
    return orient3d_indirect_IEEE_t<bigfloat, bigfloat>(
        p1,
        p2x,
        p2y,
        p2z,
...
template <class PT, class T>
static inline int orient3d_indirect_IIEE_t(
    const genericPoint& p1,
    const genericPoint& p2,
    const PT& p3x,
    const PT& p3y,
...
inline int orient3d_indirect_IIEE(
    const genericPoint& p1,
    const genericPoint& p2,
    const double& p3x,
    const double& p3y,
...
{
    int ret;
    if ((ret = orient3d_indirect_IIEE_t<interval_number, interval_number>(
             p1,
             p2,
             p3x,
             p3y,
...
template <class PT, class T>
static inline int orient3d_indirect_IIIE_t(
    const genericPoint& p1,
    const genericPoint& p2,
    const genericPoint& p3,
    const PT& p4x,
...
inline int orient3d_indirect_IIIE(
    const genericPoint& p1,
    const genericPoint& p2,
    const genericPoint& p3,
    const double& p4x,
...
{
    int ret;
    if ((ret = orient3d_indirect_IIIE_t<interval_number, interval_number>(
             p1,
             p2,
             p3,
             p4x,
...
template <class PT, class T>
static inline int orient3d_indirect_IIII_t(
    const genericPoint& p1,
    const genericPoint& p2,
    const genericPoint& p3,
    const genericPoint& p4)
...
inline int orient3d_indirect_IIII(
    const genericPoint& p1,
    const genericPoint& p2,
    const genericPoint& p3,
    const genericPoint& p4)
{
    int ret;
    if ((ret = orient3d_indirect_IIII_t<interval_number, interval_number>(p1, p2, p3, p4)) != 0)
        return ret;
