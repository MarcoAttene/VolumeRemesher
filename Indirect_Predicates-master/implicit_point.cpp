...
inline int orient2d_IEE(const genericPoint& a, const genericPoint& b, const genericPoint& c) { return orient2d_indirect_IEE(a.toSSI(), b.toExplicit2D().X(), b.toExplicit2D().Y(), c.toExplicit2D().X(), c.toExplicit2D().Y()); }
...
bool genericPoint::getApproxXYCoordinates(double& x, double& y, bool apap) const
{
	...
	if (is3D())
	{
		...
		else if (isLNC())
		{
			if (apap)
			{
				if (!toLNC().apapExplicit(op)) return false;
			}
			else
			{
				if (!toLNC().approxExplicit(op)) return false;
			}
		}
		x = p->X(); y = p->Y();
		return true;
