...
public:
  ...
  void init(const double* vertex_coords, size_t _num_vertices, const uint32_t* tet_indices, size_t _num_tets);
...
protected:
    ...
    // vi is the vertex index in 'vertices', ct is the previously inserted tetrahedron
    void insertExistingVertex(const uint32_t vi, uint64_t& ct);

    // Same as above, but this simply splits tetrahedra without making them Delaunay
    ...
    bool vertexNotCoplanar(uint32_t f1, uint32_t f2, uint32_t f3, uint32_t v_id) const;
...
public:
    ...
    void checkMesh() const {
        ...
        for (i = 0; i < tet_num; i++) {
            ...
            if (tn[3] != INFINITE_VERTEX && orient3d(vertices[tn[0]].coord, vertices[tn[1]].coord, vertices[tn[2]].coord, vertices[tn[3]].coord) <= 0) ip_error("Inverted/degn tet\n");
            if (tn[0] == tn[1] || tn[0] == tn[2] || tn[1] == tn[2]) ip_error("Wrong tet node indexes!\n");
        }
        // Check neighbors
        for (i = 0; i < tet_num * 4; i++)
            if (tet_neigh[tet_neigh[i]] != i) ip_error("Wrong neighbor!\n");
        // Check vt*
        for (i = 0; i < num_vertices; i++) {
            const uint32_t* tn = tet_node + vertices[i].inc_tet * 4;
            if (tn[0] != i && tn[1] != i && tn[2] != i && tn[3] != i) ip_error("Wrong vt*!\n");
        }
    }
};
