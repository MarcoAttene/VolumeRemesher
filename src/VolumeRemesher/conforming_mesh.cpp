...
//-------------------------------------
namespace vol_rem {
static inline void extract_tetVrts(uint32_t* tet_vrts, uint64_t tet_ind, const TetMesh* mesh)
{
...
static inline void
extract_tetFaceVrts(uint32_t* tetFace_vrts, uint64_t tet_ind, uint64_t ID, const TetMesh* mesh)
{
...
static inline void extract_tetEdgeVrts(
    uint32_t* tetEdge_vrts,
    uint64_t tet_ind,
    uint64_t ID1,
    uint64_t ID2,
    const TetMesh* mesh)
...
static inline uint32_t vrt_pointInInnerSegment(
    uint32_t pt_ind,
    uint32_t endpt0_ind,
    uint32_t endpt1_ind,
    const TetMesh* mesh)
{
    return (
        pt_ind != endpt0_ind && pt_ind != endpt1_ind &&
        pointInInnerSegment(
            mesh->vertices[pt_ind].coord,
            mesh->vertices[endpt0_ind].coord,
            mesh->vertices[endpt1_ind].coord));
}

static inline uint32_t
vrt_pointInSegment(uint32_t pt_ind, uint32_t endpt0_ind, uint32_t endpt1_ind, const TetMesh* mesh)
{
    return (
        pt_ind == endpt0_ind || pt_ind == endpt1_ind ||
        pointInSegment(
...
static inline uint32_t vrt_pointInInnerTriangle(
    uint32_t pt_ind,
...
{
    return (
        pt_ind != tri_vrt0_ind && pt_ind != tri_vrt1_ind && pt_ind != tri_vrt2_ind &&
...
static inline uint32_t vrt_pointInTriangle(
    uint32_t pt_ind,
    uint32_t tri_vrt0_ind,
    uint32_t tri_vrt1_ind,
    uint32_t tri_vrt2_ind,
...
static inline uint32_t vrt_innerSegmentsCross(
    uint32_t endpt0sgmA_ind,
...
static inline uint32_t vrt_innerSegmentCrossesInnerTriangle(
    ...
    const TetMesh* mesh)
{
    if (endpt0_ind == tri_vrt0_ind || endpt1_ind == tri_vrt0_ind || endpt0_ind == tri_vrt1_ind ||
        endpt1_ind == tri_vrt1_ind || endpt0_ind == tri_vrt2_ind || endpt1_ind == tri_vrt2_ind)
        return 0;
    ...
    return innerSegmentCrossesInnerTriangle(e0_coord, e1_coord, t0_coord, t1_coord, t2_coord);
}

static inline uint32_t vrt_innerSegmentCrossesTriangle(
    uint32_t endpt0_ind,
    uint32_t endpt1_ind,
    uint32_t tri_vrt0_ind,
    uint32_t tri_vrt1_ind,
    ...
    const TetMesh* mesh)
{
    ...
    const double* t1_coord = mesh->vertices[tri_vrt1_ind].coord;
    const double* t2_coord = mesh->vertices[tri_vrt2_ind].coord;
    return innerSegmentCrossesTriangle(e0_coord, e1_coord, t0_coord, t1_coord, t2_coord);
}

static inline uint32_t vrt_innerTrianglesCrosses(
    uint32_t triA_vrt0_ind,
    ...
    const TetMesh* mesh)
{
    if (vrt_innerSegmentCrossesInnerTriangle(
            triA_vrt0_ind,
            triA_vrt1_ind,
            triB_vrt0_ind,
            triB_vrt1_ind,
            triB_vrt2_ind,
            mesh) ||
        vrt_innerSegmentCrossesInnerTriangle(
            triA_vrt1_ind,
            triA_vrt2_ind,
            triB_vrt0_ind,
            triB_vrt1_ind,
...
uint32_t
vrt_signe_orient3d(uint32_t vrt1, uint32_t vrt2, uint32_t vrt3, uint32_t vrt4, const TetMesh* mesh)
{
    if (vrt1 == vrt2 || vrt1 == vrt3 || vrt1 == vrt4 || vrt2 == vrt3 || vrt2 == vrt4 ||
        vrt3 == vrt4)
        return 0;
...
static inline uint32_t vrt_same_half_plane(
    uint32_t test_vrt0,
    uint32_t test_vrt1,
    ...
    const TetMesh* mesh)
{
    if (test_vrt0 == strLine4_vrt0 || test_vrt0 == strLine4_vrt1 || test_vrt1 == strLine4_vrt0 ||
        test_vrt1 == strLine4_vrt1)
        return 0;
...
static inline uint32_t
tetvrtONconstraintplane(uint32_t vrt, const uint32_t* c_vrts, const TetMesh* mesh)
{
    return (vrt_signe_orient3d(c_vrts[0], c_vrts[1], c_vrts[2], vrt, mesh) == 0);
}

static inline uint32_t
tetedgeONconstraintplane(const uint32_t* te_vrts, const uint32_t* c_vrts, const TetMesh* mesh)
{
    return (
        tetvrtONconstraintplane(te_vrts[0], c_vrts, mesh) &&
        tetvrtONconstraintplane(te_vrts[1], c_vrts, mesh));
}

static inline uint32_t
tetfaceONconstraintplane(const uint32_t* tf_vrts, const uint32_t* c_vrts, const TetMesh* mesh)
{
    return (
        tetvrtONconstraintplane(tf_vrts[0], c_vrts, mesh) &&
...
static inline uint32_t vrtsInSameHalfSpace(
    uint32_t test0,
    uint32_t test1,
    uint32_t p0,
    uint32_t p1,
...
{
    return (
        vrt_signe_orient3d(p0, p1, p2, test0, mesh) == vrt_signe_orient3d(p0, p1, p2, test1, mesh));
}
...
// otherwise returns 0.
static inline bool arrayUINT32_contains_elem(const uint32_t* array, uint32_t lenght, uint32_t elem)
{
...
//         0 otherwise.
static inline bool is_vertex_ofTet(uint32_t vrt, uint64_t tet_ID, const TetMesh* mesh)
{
    return (
        mesh->tet_node[tet_ID] == vrt || mesh->tet_node[tet_ID + 1] == vrt ||
        mesh->tet_node[tet_ID + 2] == vrt || mesh->tet_node[tet_ID + 3] == vrt);
}
...
{
    return (
        arrayUINT32_contains_elem(tri_vrts, 3, v0) && arrayUINT32_contains_elem(tri_vrts, 3, v1));
}
...
bool segment_in_triSide(
    uint32_t v0,
    uint32_t v1,
    const uint32_t* tri_vrts,
    uint32_t* opp_tri_vrt,
    const TetMesh* mesh)
{
...
//         0 otherwise.
static inline bool
triangle_is_tetFace(uint32_t v0, uint32_t v1, uint32_t v2, uint64_t tet_ID, const TetMesh* mesh)
{
    return (
        is_vertex_ofTet(v0, tet_ID, mesh) && is_vertex_ofTet(v1, tet_ID, mesh) &&
...
static inline uint32_t
tet_faceID(uint32_t v0, uint32_t v1, uint32_t v2, uint64_t tet_ID, const TetMesh* mesh)
{
...
bool triangle_in_VT(uint32_t v0, uint32_t v1, uint32_t v2, TetMesh* mesh, uint64_t* tet_face_ind)
{
    bool found = false;
...
uint32_t opposite_vertex_face(const TetMesh* mesh, uint64_t tet_ind, const uint32_t* face_vrts)
{
...
void opposite_face_vertices(const TetMesh* mesh, uint64_t tet_ind, uint32_t v_ind, uint32_t* ptr)
{
...
uint64_t adjTet_oppsiteTo_vertex(const TetMesh* mesh, uint64_t tet_ind, uint32_t v_ind)
{
    if (mesh->tet_node[4 * tet_ind] == v_ind) return mesh->tet_neigh[4 * tet_ind] >> 2;
    if (mesh->tet_node[4 * tet_ind + 1] == v_ind) return mesh->tet_neigh[4 * tet_ind + 1] >> 2;
    if (mesh->tet_node[4 * tet_ind + 2] == v_ind) return mesh->tet_neigh[4 * tet_ind + 2] >> 2;
    if (mesh->tet_node[4 * tet_ind + 3] == v_ind) return mesh->tet_neigh[4 * tet_ind + 3] >> 2;
    return UINT64_MAX;
}
...
void opposite_side_vertices(
    const TetMesh* mesh,
    uint64_t tet_ind,
    const uint32_t* u_inds,
    uint32_t* ptr)
{
    uint32_t j = 0, v;
...
void enqueueTetsArray(
    const uint64_t* arrayOfTetsA,
    uint64_t lenghtA,
    uint64_t** arrayOfTetsB,
    uint64_t* lenghtB)
{
    if (*lenghtB == 0) {
        *lenghtB = lenghtA;
        *arrayOfTetsB = (uint64_t*)malloc(sizeof(uint64_t) * (*lenghtB));
        for (uint64_t j = 0; j < lenghtA; j++) (*arrayOfTetsB)[j] = arrayOfTetsA[j];
    } else {
        uint64_t tmp = *lenghtB;
        *lenghtB += lenghtA;
        *arrayOfTetsB = (uint64_t*)realloc(*arrayOfTetsB, sizeof(uint64_t) * (*lenghtB));
        for (uint64_t j = 0; j < lenghtA; j++) (*arrayOfTetsB)[tmp + j] = arrayOfTetsA[j];
    }
}
...
//         0 otherwise.
bool tri_onSameEdge_allCoPlanar(
    uint32_t he0,
    uint32_t he1,
    const half_edge_t* half_edges,
    ...
    const TetMesh* mesh)
{
    ...
    while (u == hp0 || u == hp1) u = constraints->tri_vertices[tri_id + (++k)];
    ...
    for (uint32_t he = he0 + 1; he <= he1; he++) {
        ...
        if (hp0 != v1 && hp1 != v1)
            ...
            u = v2;
        for (i = j = 0; i < 3; i++)
            if (i != dom) oc[j++] = mesh->vertices[u].coord[i];
...
uint32_t intersectionClass_tetFace_constr(const uint32_t* c, const uint32_t* t, const TetMesh* mesh)
{
...
uint32_t intersectionClass_tetEdge_constr(
    ...
    const TetMesh* mesh)
{
    ...
    if (opptE_same_or) {
        ...
        if (!tE0_in && !tE1_in)
            printf(
                "[conforming_mesh.c]intersectionClass_tetEdge_constr: ERROR "
        ...
        if (tE0_in && tE1_in) {
            ...
            // (boundary included)
            // THEN the intersection is IMPROPER.
            if (vrt_innerSegmentCrossesTriangle(
                    opptE_vrts[0],
                    opptE_vrts[1],
                    c_vrts[0],
                    c_vrts[1],
...
uint32_t intersectionClass_tetVrt_constr(
    const uint32_t* c_vrts,
    uint32_t tV,
    ...
    const TetMesh* mesh)
{
    ...
    if (opptF_same_or)
        ...
        // THEN the intersection is IMPROPER
        if (!vrt_pointInTriangle(tV, c_vrts[0], c_vrts[1], c_vrts[2], mesh)) return 1;
        // IF tv is in the constraint interior
        // THEN the intersection is IMPROPER
        if (vrt_pointInInnerTriangle(tV, c_vrts[0], c_vrts[1], c_vrts[2], mesh)) return 1;
...
void find_improperIntersection(
    const uint32_t* constr_v,
    const uint64_t* tets,
    uint64_t num_tets,
    ...
    const TetMesh* mesh)
{
    for (uint64_t tet_i = 0; tet_i < num_tets; tet_i++) {
        ...
        if (tet_v[3] == INFINITE_VERTEX)
            ...
            or_tet_v[3] = vrt_signe_orient3d(constr_v[0], constr_v[1], constr_v[2], tet_v[3], mesh);
        ...
        if (num_coplan_v == 2) {
            ...
            iclass =
                intersectionClass_tetEdge_constr(constr_v, tetEdge_v, oppTetEdge_v, same_or, mesh);
        ...
        if (num_coplan_v == 1) {
            ...
            if (oppTetFace_v[0] == oppTetFace_v[1] && oppTetFace_v[0] == oppTetFace_v[2])
                continue; // same_or = 1; <-----------------------

            iclass = intersectionClass_tetVrt_constr(
                constr_v,
                tet_u,
                oppTetFace_v,
                or_oppTetFace_v,
...
//         0 otherwise.
uint32_t properIntersection_tetFace_constr(
    const uint32_t* c_vrts,
...
uint32_t properIntersection_tetEdge_constr(
    const uint32_t* c_vrts,
    const uint32_t* tE_vrts,
    const uint32_t* opptE_vrts,
    const TetMesh* mesh)
{
    ...
    uint32_t or_opptE0 = vrt_signe_orient3d(c_vrts[0], c_vrts[1], c_vrts[2], opptE_vrts[0], mesh);
    uint32_t or_opptE1 = vrt_signe_orient3d(c_vrts[0], c_vrts[1], c_vrts[2], opptE_vrts[1], mesh);
    ...
    if (or_opptE0 == 0) {
        ...
        if (segment_in_triSide(tE_vrts[0], tE_vrts[1], c_vrts, &opp_c_vrt, mesh))
            if (!vrt_same_half_plane(opptE_vrts[0], opp_c_vrt, tE_vrts[0], tE_vrts[1], mesh))
                return 1;
    ...
    if (or_opptE1 == 0) {
        uint32_t opp_c_vrt;
        if (segment_in_triSide(tE_vrts[0], tE_vrts[1], c_vrts, &opp_c_vrt, mesh))
            if (!vrt_same_half_plane(opptE_vrts[1], opp_c_vrt, tE_vrts[0], tE_vrts[1], mesh))
                return 1;
        // otherwise there is no proper intersection of dimension 1..
        return 0;
    }
...
uint32_t properIntersection_tetVrt_constr(
    const uint32_t* c_vrts,
    uint32_t tV,
    const uint32_t* opptF_vrts,
    const TetMesh* mesh)
{
    ...
    uint32_t or_opptF0 = vrt_signe_orient3d(c_vrts[0], c_vrts[1], c_vrts[2], opptF_vrts[0], mesh);
    uint32_t or_opptF1 = vrt_signe_orient3d(c_vrts[0], c_vrts[1], c_vrts[2], opptF_vrts[1], mesh);
    uint32_t or_opptF2 = vrt_signe_orient3d(c_vrts[0], c_vrts[1], c_vrts[2], opptF_vrts[2], mesh);
    ...
    // Condition (iii)
    if (vrt_innerTrianglesCrosses(c_vrts[0], c_vrts[1], c_vrts[2], tV, tV_OVER, tV_UNDER1, mesh))
        return 0;

    // Condition (iv)
    if (vrt_innerTrianglesCrosses(c_vrts[0], c_vrts[1], c_vrts[2], tV, tV_OVER, tV_UNDER2, mesh))
        return 0;

    // Condition (v)
    if (vrt_innerTrianglesCrosses(
            c_vrts[0],
            c_vrts[1],
            c_vrts[2],
            tV_OVER,
...
// Note. It is assumed that each tetrahedron of tet_list intersects
//       (properly or improperly) the constraint.
void improperIntersection(
    const uint32_t* constr_vrts,
    const uint64_t* tet_list,
    uint64_t num_tet_list,
    uint32_t* mark_TetIntersection,
    const TetMesh* mesh)
{
    for (uint64_t list_ind = 0; list_ind < num_tet_list; list_ind++) {
        ...
        if (tetVrts[3] == UINT32_MAX) {
            ...
            if (properIntersection_tetFace_constr(constr_vrts, tetFace_vrts, mesh)) continue;
            ...
            if (properIntersection_tetEdge_constr(constr_vrts, tetEdge_vrts, oppTetEdge_vrts, mesh))
                continue;
            ...
            if (properIntersection_tetVrt_constr(constr_vrts, tetVrt, oppTetFace_vrts, mesh))
                continue;
        ...
        tetVrt = tetVrts[0];
        extract_tetFaceVrts(oppTetFace_vrts, tet_ind, 0, mesh);
        if (properIntersection_tetVrt_constr(constr_vrts, tetVrt, oppTetFace_vrts, mesh)) continue;
...
static inline void fill_connecting_vrts(
    ...
    uint64_t* num_intersecated_tet)
{
    ...
    for (uint64_t i = 0; i < num_incTet; i++)
        INSERT_TET_IN_LIST(incTet[i], newTetIn_incTet, num_newTetIn_incTet, mark_TetIntersection);
    ...
    for (uint64_t i = 0; i < num_incTet; i++) {
        ...
        if (vrt_pointInInnerSegment(v_oppf_inds[2], v_curr, v_stop, mesh)) {
            fill_connecting_vrts(connecting_vrts, 1, v_oppf_inds[2], UINT32_MAX, UINT32_MAX);
            break;
        }
    ...
    uint64_t* num_intersecated_tet)
{
    ...
    for (uint64_t ind = 0; ind < num_incTet; ind++) {
        ...
        // to tet_cutted_edge.
        // Check also if it is the right diretction: against v_start.
        if (vrt_innerSegmentsCross(v_start, v_stop, v_oppe_inds[0], v_oppe_inds[1], mesh) &&
            vrtsInSameHalfSpace(
                tet_cutted_edge[0],
                v_start,
                v_oppe_inds[0],
                ...
                tet_cutted_edge[1],
                mesh)) {
            fill_connecting_vrts(connecting_vrts, 2, v_oppe_inds[0], v_oppe_inds[1], UINT32_MAX);
            break;
        }
        ...
        if (vrt_pointInInnerSegment(v_oppe_inds[0], v_start, v_stop, mesh) &&
            vrt_same_half_plane(
                ...
                tet_cutted_edge[1],
                mesh)) {
            fill_connecting_vrts(connecting_vrts, 1, v_oppe_inds[0], UINT32_MAX, UINT32_MAX);
            break;
        }
        ...
        if (vrt_pointInInnerSegment(v_oppe_inds[1], v_start, v_stop, mesh) &&
            vrt_same_half_plane(
                ...
                tet_cutted_edge[1],
                mesh)) {
            fill_connecting_vrts(connecting_vrts, 1, v_oppe_inds[1], UINT32_MAX, UINT32_MAX);
            break;
        }
        ...
        // Check also if it is the right diretction: against v_start.
        if (vrt_innerSegmentsCross(v_start, v_stop, tet_cutted_edge[0], v_oppe_inds[0], mesh) &&
            vrt_same_half_plane(
                v_oppe_inds[0],
                v_stop,
                tet_cutted_edge[0],
        ...
        // Check also if it is the right diretction: against v_start.
        if (vrt_innerSegmentsCross(v_start, v_stop, tet_cutted_edge[0], v_oppe_inds[1], mesh) &&
            vrt_same_half_plane(
                v_oppe_inds[1],
                v_stop,
                tet_cutted_edge[0],
        ...
        // Check also if it is the right diretction: against v_start.
        if (vrt_innerSegmentsCross(v_start, v_stop, tet_cutted_edge[1], v_oppe_inds[0], mesh) &&
            vrt_same_half_plane(
                v_oppe_inds[0],
                v_stop,
                tet_cutted_edge[0],
        ...
        // Check also if it is the right diretction: against v_start.
        if (vrt_innerSegmentsCross(v_start, v_stop, tet_cutted_edge[1], v_oppe_inds[1], mesh) &&
            vrt_same_half_plane(
                v_oppe_inds[1],
                v_stop,
                tet_cutted_edge[0],
    ...
    uint64_t* nextTet_ind)
{
    ...
    *nextTet_ind = opp_tet_ind;
    ...
    if (vrt_innerSegmentsCross(v_start, v_stop, tet_crossed_face[0], v_opp_ind, mesh)) {
        fill_connecting_vrts(connecting_vrts, 2, tet_crossed_face[0], v_opp_ind, UINT32_MAX);
        return isNew;
    }

    // Edge <tet_crossed_face[1], v_opp_ind>
    if (vrt_innerSegmentsCross(v_start, v_stop, tet_crossed_face[1], v_opp_ind, mesh)) {
        fill_connecting_vrts(connecting_vrts, 2, tet_crossed_face[1], v_opp_ind, UINT32_MAX);
        return isNew;
    }

    // Edge <tet_crossed_face[2], v_opp_ind>
    if (vrt_innerSegmentsCross(v_start, v_stop, tet_crossed_face[2], v_opp_ind, mesh)) {
        fill_connecting_vrts(connecting_vrts, 2, tet_crossed_face[2], v_opp_ind, UINT32_MAX);
        return isNew;
    }
...
uint32_t tet_intersects_triInterior(
    uint64_t tet_ind,
    const uint32_t* c,
    const TetMesh* mesh,
...
//         by using mark_TetIntersection marks the intersection as explained
//           at the beginning of the function insert_constraints.
void intersections_constraint_sides(
    TetMesh* mesh,
    ...
    uint32_t* mark_TetIntersection)
{
    ...
    for (uint32_t constr_side = 0; constr_side < 3; constr_side++) {
        ...
        enqueueTetsArray(found_tet, num_found_tet, intersecatedTet, num_intersecatedTet);
        free(found_tet);
        found_tet = NULL;
...
static inline uint32_t constrInterior_found(
    const TetMesh* mesh,
...
//         if 1 is rerurned.
uint32_t constrInterior_firstStep(
    const TetMesh* mesh,
    const uint32_t* constraints_vrts,
    ...
    uint32_t* mark_TetIntersection)
{
    ...
    for (uint64_t i = 0; i < 4; i++) {
        ...
        if (constrInterior_found(mesh, constraints_vrts, adj_tet_ind, mark_TetIntersection))
            num_tets_intersects +=
                constrInterior_count(mesh, constraints_vrts, adj_tet_ind, mark_TetIntersection);
    }
    ...
    uint64_t* tet_list)
{
    ...
    for (uint64_t i = 0; i < 4; i++) {
        ...
        if (mark_TetIntersection[adj_tet_ind] == IMPROPER_INTERSECTION_COUNTED ||
            ...
            constrInterior_save(mesh, adj_tet_ind, mark_TetIntersection, pos, tet_list);
    }
    return;
}
...
//         by using mark_TetIntersection marks the intersection as explained
//         at the beginning of the function insert_constraints.
void intersections_constraint_interior(
    TetMesh* mesh,
    const uint32_t* constraint_vrts,
    uint64_t* num_intersecatedTet,
    uint64_t** intersecatedTet,
    uint32_t* mark_TetIntersection)
{
    ...
    for (uint64_t side_tet_ind = 0; side_tet_ind < num_bnd_tets; side_tet_ind++) {
        ...
        uint64_t num = constrInterior_count(mesh, constraint_vrts, adjIN_tet, mark_TetIntersection);
        ...
        if (num > 1) {
            ...
            enqueueTetsArray(interiorConstrInterct_tet, num, intersecatedTet, num_intersecatedTet);
...
//
static inline void
compile_map_innerInt(uint32_t tri_ind, uint64_t tet_ind, uint32_t* num_map, uint32_t** map)
{
...
// Output:
static inline void
compile_map_fi(uint32_t tri_ind, uint64_t tet_ind, uint32_t* num_map_fi, uint32_t** map_fi)
{
    ...
    if (num_map_fi[tet_ind] == 1) // It is the first overlapping constraint.
        ...
        map_fi[tet_ind] =
            (uint32_t*)realloc(map_fi[tet_ind], sizeof(uint32_t) * num_map_fi[tet_ind]);
...
//  Input:
// Output:
static inline void compile_tetfFaces_map(
    uint32_t tri_ind,
    ...
    uint32_t** map_f3)
{
    ...
    for (uint32_t tri_ind = 0; tri_ind < constraints->num_triangles; tri_ind++) {
        ...
        intersections_constraint_sides(
            mesh,
            v,
            &num_intersecatedTet,
            &intersecatedTet,
        ...
        intersections_constraint_interior(
            mesh,
            v,
            &num_intersecatedTet,
            &intersecatedTet,
