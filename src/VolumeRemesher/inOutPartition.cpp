...
bool isFirstConnCellBelowFace(BSPface& f, BSPcomplex* cpx)
{
    ...
    for (uint64_t ei : f.edges)
        cpx->vrts_visit[cpx->edges[ei].vertices[0]] = cpx->vrts_visit[cpx->edges[ei].vertices[1]] =
            1;
...
void BSPcomplex::markInternalCells(
    ...
    const std::vector<double>& face_areas)
{
    ...
    for (size_t i = 0; i < faces.size(); i++) {
        ...
        const uint64_t cell1 = f.conn_cells[0];
        if (!isSkinFace(f, skin_colour)) {
            // 'w' is an additional weight for arcs that promotes the cut of arcs corresp. to faces
            // having all their vertices on the boundary of the input surface (i.e. promote hole
            // filling)
            double w = 0.1;
            for (uint64_t eid : f.edges)
            ...
            gc.setNeighbors(
                (GCoptimization::SiteID)cell1,
                (GCoptimization::SiteID)cell2,
                face_areas[i] * w);
        }
    }

    const double int_weight =
        0.1; // Internal cell penalization less than external to avoid artifacts at intersections
    for (size_t i = 0; i < cells.size(); i++)
        gc.setDataCost((GCoptimization::SiteID)i, 0, cell_costs_external[i]);
    for (size_t i = 0; i < cells.size(); i++)
        gc.setDataCost((GCoptimization::SiteID)i, 1, cell_costs_internal[i] * int_weight);
