...
#else
void PrintMemoryInfo() {
  printf("Peak memory usage unavailable. This feature is implemented on "
         "Windows/MSVC only.\n");
}
#endif
...
void read_nodes_and_constraints(const double *coords_A, uint32_t npts_A,
                                const uint32_t *tri_idx_A, uint32_t ntri_A,
...
// void saveVRML(const char* filename, double* coords_A, uint32_t npts_A,
// uint32_t* tri_idx_A, uint32_t ntri_A, bool wireframe)
//{
...
BSPcomplex *makePolyhedralMesh(const char *fileA_name, double *coords_A,
                               ...
                               uint32_t ntri_A, const char *fileB_name,
                               double *coords_B, uint32_t npts_B,
                               uint32_t *tri_idx_B, uint32_t ntri_B,
                               char bool_opcode, bool free_mem, bool verbose,
                               bool logging) {
  ...
  if (logging) {
    ...
    if (logfile == NULL) {
      ...
      logger(logfile, "Filename,Delaunay,Virtual,Mapping,Total "
                      "Delaunay,Conversion,Subdivision,Face "
                      "Coloring,In-Out,Total BSP,TOTAL,Mem\n");
    } else
      logfile = freopen("mesh_generator.log", "a", logfile);

    if (two_input)
      logger(logfile, "%s_%c_%s,", fileA_name, bool_opcode, fileB_name);
    else
      logger(logfile, "%s,", fileA_name);
    fflush(logfile);
  }
  ...
  BSPcomplex *complex_p = new BSPcomplex(
  ...
  while (cell_ind < complex.cells.size()) {
    ...
    if ((cell_ind & 0x0000000000000fff) == 0) {
      printf("\r%llu cells created                  ", cell_ind);
      fflush(stdout);
      if (getPeakMegabytesUsed() > MEM_THRESHOLD)
        ip_error("Maximum memory reached\n");
    }
#endif
  ...
  if (logging) {
    logger(logfile, "%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f\n",
           (double)(time2 - time1) / CLOCKS_PER_SEC,
           (double)(time3 - time2) / CLOCKS_PER_SEC,
           (double)(time4 - time3) / CLOCKS_PER_SEC, DEL_time,
           (double)(time5 - time4) / CLOCKS_PER_SEC,
           (double)(time6 - time5) / CLOCKS_PER_SEC,
           (double)(time7 - time6) / CLOCKS_PER_SEC,
           (double)(time8 - time7) / CLOCKS_PER_SEC, BSP_time,
           BSP_time + DEL_time
#ifdef _MSC_VER
           ,
           getPeakMegabytesUsed()
  ...
  // double* coord;
  // complex.extractSkinTriMesh("input4.wrl", bool_opcode, &coord, &npts,
  // &tri_idx, &ntri); saveVRML("input4.wrl", coord, npts, tri_idx, ntri,
  // false); saveVRML("input5.wrl", coord, npts, tri_idx, ntri, true); exit(0);

  return complex_p;
}

BSPcomplex *remakePolyhedralMesh(const double *coords_A, uint32_t npts_A,
                                 const uint32_t *tri_idx_A, uint32_t ntri_A,
                                 const double *coords_B, uint32_t npts_B,
                                 const uint32_t *tet_idx_B, uint32_t ntet_B,
                                 bool verbose, bool no_class) {
  if (verbose)
  ...
  read_nodes_and_constraints(coords_A, npts_A, tri_idx_A, ntri_A, &vertices,
                             &num_vertices, &constraints->tri_vertices,
                             &constraints->num_triangles, verbose);
  constraints->constr_group =
      (uint32_t *)calloc(constraints->num_triangles, sizeof(uint32_t));
  ...
  mesh->vertices = (vertex_t *)realloc(mesh->vertices,
                                       mesh->num_vertices * sizeof(vertex_t));
  std::memcpy(mesh->vertices + orig_numv, vertices,
  ...
  for (uint32_t i = 0; i < constraints->num_triangles * 3; i++) {
    constraints->tri_vertices[i] =
        mesh->vertices[constraints->tri_vertices[i]].original_index;
  }

  // Here we may still have unused vertices (copies of coincident vertices that
  // do not belong to tets) Should clean... here below a sketch of the algorithm
  ...
  // 1) Create another vector<uint32_t> 'map' as large as mesh->vertices and
  // init it with increasing indexes 2) for each vertex v = vertices[i],
  // decrease map[i] by offset
  ...
  // 4) replace each connstraints->tri_vertices[i] with
  // map[constraints->tri_vertices[i]]
  ...
  for (uint64_t face_ind = 0; face_ind < complex.faces.size(); face_ind++) {
    BSPface &face = complex.faces[face_ind];
    if (face.colour == GREY)
      face.colour = complex.blackAB_or_white(face_ind, false);
  }
