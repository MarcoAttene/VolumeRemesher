...
// Output: nothing.
// Note. face is removed from cells[cell_ind] and assigned to
// cells[newCell_ind].
...
//         of the BSPcell to which the BSPface belong to: cell_vrts.
// Output: nothing.
void BSPcomplex::add_commonFace(uint32_t constr, uint64_t cell_ind,
                                uint64_t newCell_ind,
                                const vector<uint32_t> &cell_vrts,
...
//
//
void BSPcomplex::saveMesh(const char *filename, const char bool_opcode,
                          bool tetrahedrize) {
  ofstream f(filename);

  if (!f)
    ip_error("\nBSPcomplex::[BSP.cpp]saveTetMesh: FATAL ERROR cannot open the "
             "file.\n");
  ...
  if (tetrahedrize) {
    ...
    for (size_t i = 0; i < vrts_visit.size(); i++)
      vrts_visit[i] = 0;
    for (uint32_t t = 0; t < final_tets.size(); t++)
      vrts_visit[final_tets[t]] = 1;
    ...
    for (size_t i = 0; i < vrts_visit.size(); i++)
      vrts_visit[i] = 0;
    for (size_t i = 0; i < edge_visit.size(); i++)
      edge_visit[i] = 0;
    for (BSPcell &cell : cells)
      if (cell.place == INTERNAL_A) {
        internal_cell_num++;
