...
TetMesh::TetMesh()
    ...
    , tet_size(0)
    , tet_num_vertices(0)
    , mark_tetrahedra(NULL)
    , Del_size_tmp(1024)
    , Del_num_tmp(0)
    , Del_tmp(NULL)
    , Del_size_deleted(1024)
    , Del_num_deleted(0)
    , Del_deleted(NULL)
    , Del_buffer(NULL)
{}
...
void TetMesh::init()
{
    ...
    if (ori == 0.0) ip_error("Input vertices do not define a volume.\n");
...
void TetMesh::saveTET(const char* filename)
{
    ...
    for (uint64_t i = 0; i < tet_num; i++)
        if (tet_node[i * 4 + 3] != INFINITE_VERTEX) ngnt++;
    ...
    for (uint64_t i = 0; i < tet_num; i++)
        if (tet_node[i * 4 + 3] != INFINITE_VERTEX)
            f << "4 " << tet_node[i * 4] << " " << tet_node[i * 4 + 1] << " " << tet_node[i * 4 + 2]
              << " " << tet_node[i * 4 + 3] << "\n";
...
{
    if (tet_node[tet + 3] == INFINITE_VERTEX) tet = getNeighbor(tet, 3);
...
static inline double symbolicPerturbation(
    uint32_t indices[5],
    const double* i,
    const double* j,
    const double* k,
...
void TetMesh::bnd_push(uint32_t v_id, uint32_t node1, uint32_t node2, uint32_t node3, uint64_t bnd)
{
...
void TetMesh::tetrahedrizeHole(uint64_t* tet)
{
    ...
    for (uint64_t i = 0; i < blength; i++) {
        ...
        if (tet_node[tet + 3] != INFINITE_VERTEX) // Mod.1
            for (uint32_t j = 0; j < 4; j++) vertices[tet_node[tet + j]].inc_tet = tet >> 2;
    }
...
void count_incTet(
    uint64_t tet_ind,
    const uint32_t central_vertex_ind,
    TetMesh* mesh,
    uint64_t* tot_incTet)
{ // Mod.1
...
void save_incTet(uint64_t tet_ind, TetMesh* mesh, uint64_t* incTet, uint64_t* pos)
{ // Mod.1
    for (uint32_t i = 0; i < 4; i++) {
...
// Output: by using num_incTet returns the number of non-ghost tetrahedra incident in
// central_vertex,
//         returns an array containing the indices of non-ghost tetrahedra incident in
//         central_vertex.
uint64_t* TetMesh::incident_tetrahedra(
    const uint32_t central_vertex_ind,
...
uint64_t* TetMesh::ETrelation(
    const uint32_t* edge_ends,
    ...
    uint64_t* length_related_tet) const
{
    ...
    // other_tet_vrts_ind[0] is the index of the common vertex (not on the edge) between tet0 and
    // tet1, other_tet_vrts_ind[0] is the index of the vertex opposite to the face between tet1 and
    // tet2
    for (uint64_t j = 0; j < 4; j++)
        if (tet_node[4 * curr_tet_ind + j] == other_tet_vrts_ind[0]) {
            next_move_dir =
                j; // is the face-ID of tet1 -> tet2, and vrt-ID opposite to that face [wrt tet1]
            break;
        }
    ...
    while (curr_tet_ind != tet0_ind) {
        ...
        // Find the vrt-ID of the common vertex (not on the edge) between curr_tet and next_tet [wrt
        // curr_tet].
        uint64_t tmp = tet_neigh[4 * prec_tet_ind + move_dir] & 3;
        // Find the index of the vertex of curr_tet opposite to the face between curr_tet and
        // prec_tet.
        ...
        // Find the face-ID of the common face between new curr_tet and new prec_tet [wrt new
        // curr_tet].
        for (uint64_t j = 0; j < 4; j++)
            if (tet_node[4 * curr_tet_ind + j] == v_ind) next_move_dir = j;
    }
...
void TetMesh::insertExistingVerticesNonDelaunay(const uint32_t first)
{
    ...
    uint64_t ct = 0;
    for (uint32_t i = first; i < num_vertices; i++) insertExistingVertexNonDelaunay(i, ct);
...
//         double* crd = v->coord;
//         if (fscanf(file, "%lf %lf %lf\n", crd, crd+1, crd+2) != 3) { free(vertices);
//         fclose(file); return false; } v->original_index = i;
//     }
//
...
//         free(vertices); fclose(file); return false; } vertices[nodes[0]].inc_tet =
//         vertices[nodes[1]].inc_tet = vertices[nodes[2]].inc_tet = vertices[nodes[3]].inc_tet = i;
...
//            tet_node[last++] = f.v[2];
//            // This is ugly!
//            if (orient3d(vertices[f.v[0]].coord, vertices[f.v[1]].coord, vertices[f.v[2]].coord,
//            vertices[tet_node[f.t]].coord) > 0) {
    ...
    size_t _num_tets)
{
    ...
    for (size_t i = 0; i < _num_tets; i++) {
        ...
        vertices[nodes[0]].inc_tet = vertices[nodes[1]].inc_tet = vertices[nodes[2]].inc_tet =
            vertices[nodes[3]].inc_tet = i;
        if (orient3d(
                vertices[nodes[0]].coord,
                vertices[nodes[1]].coord,
                vertices[nodes[2]].coord,
                vertices[nodes[3]].coord) < 0)
            std::swap(nodes[0], nodes[1]);
    }
    ...
    for (size_t i = 0; i < tet_num * 4; i++) {
        ...
        if (tet_neigh[f.t] == UINT64_MAX) {
            ...
            // This is ugly!
            if (orient3d(
                    vertices[f.v[0]].coord,
                    vertices[f.v[1]].coord,
                    vertices[f.v[2]].coord,
                    vertices[tet_node[f.t]].coord) > 0) {
                std::swap(tet_node[last - 1], tet_node[last - 2]);
                std::swap(facets[facets.size() - 2].t, facets[facets.size() - 3].t);
            }
