...
namespace vol_rem {
TetMesh::TetMesh()
    : vertices(NULL), num_vertices(0), tet_node(NULL), tet_neigh(NULL),
      tet_subdet(NULL), tet_num(0), tet_size(0), tet_num_vertices(0),
      mark_tetrahedra(NULL), Del_size_tmp(1024), Del_num_tmp(0), Del_tmp(NULL),
      Del_size_deleted(1024), Del_num_deleted(0), Del_deleted(NULL),
      Del_buffer(NULL) {}
...
void TetMesh::init() {
  ...
  for (; ori == 0.0 && k < n - 1; k++)
    for (l = k + 1; ori == 0.0 && l < n; l++)
      ori = orient3d(vertices[i].coord, vertices[j].coord, vertices[k].coord,
                     vertices[l].coord);
  l--;
  k--;
  ...
  for (uint64_t f0 = 4;;) {
    ...
    for (; i < 4; i++) {
      ...
      if (i != f0 && (o3d = orient3d(a, b, c, vc)) < 0.0) {
        ...
        if (a[0] == vc[0] && a[1] == vc[1] && a[2] == vc[2]) {
          vertices[v_id].original_index = Node[(i + 1) & 3];
          return UINT64_MAX;
        }
        if (b[0] == vc[0] && b[1] == vc[1] && b[2] == vc[2]) {
          vertices[v_id].original_index = Node[(i & 2) ^ 3];
          return UINT64_MAX;
        }
        if (c[0] == vc[0] && c[1] == vc[1] && c[2] == vc[2]) {
          vertices[v_id].original_index = Node[(i + 3) & 2];
          return UINT64_MAX;
        }
      }
    }
...
static inline double symbolicPerturbation(uint32_t indices[5], const double *i,
...
double TetMesh::vertexInTetSphere(uint64_t tet, uint32_t v_id) {
  ...
  if (Node[3] == INFINITE_VERTEX) {
    ...
    if (det == 0.0) {
      ...
      if (det == 0.0)
        ip_error("Symbolic perturbation failed\n");
    }
    return det;
  }
  ...
  double det =
      aex * SubDet[0] - aey * SubDet[1] + aez * SubDet[2] - aer * SubDet[3];
  ...
  if (det == 0.0) {
    ...
    if (det == 0.0)
      ip_error("Symbolic perturbation failed! This should never happen :-(\n");
  }
  return det;
}

void TetMesh::bnd_push(uint32_t v_id, uint32_t node1, uint32_t node2,
...
void TetMesh::deleteInSphereTets(uint64_t tet, const uint32_t v_id) {
  ...
  for (start = Del_num_deleted - 1; start < Del_num_deleted; start++) {
    ...
    if (Del_num_deleted + 4 > Del_size_deleted) {
      Del_deleted = (uint64_t *)realloc(Del_deleted,
                                        2 * Del_num_deleted * sizeof(uint64_t));
...
void TetMesh::tetrahedrizeHole(uint64_t *tet) {
  ...
  if (blength > clength) {
    if (blength > Del_size_deleted) {
      Del_deleted =
          (uint64_t *)realloc(Del_deleted, 2 * blength * sizeof(uint64_t));
...
void TetMesh::compute_subDet(const uint64_t tet) {
  ...
  if (Node[3] != INFINITE_VERTEX) {
    ...
    SubDet[3] =
        SubDet[0] * SubDet[0] + SubDet[1] * SubDet[1] + SubDet[2] * SubDet[2];
  }
}
...
//         the index of the vertex in which incidences are searched:
//         central_vertex_ind, pointer to mesh, pointer to a tetraheron index
//         type: tot_incTet.
// Output: by using tot_incTet increments of one unity the number of incident
// tetrahedra each
...
//         If tot_incTet is incremented the relative tetrahedra is passed to
//         this function...
void count_incTet(uint64_t tet_ind, const uint32_t central_vertex_ind,
                  TetMesh *mesh, uint64_t *tot_incTet) { // Mod.1
...
// Output: by using incTet returns the indices of the tetrahedra icident in a
// common vertex,
//         those tetrahedra have been counted by the function count_incTet and
//         marked as 1. Face neighbour of tet is added to incTet if its marker
//         is equal to 1. pos stores the number of elements of incTet.
void save_incTet(uint64_t tet_ind, TetMesh *mesh, uint64_t *incTet,
                 uint64_t *pos) { // Mod.1
  for (uint32_t i = 0; i < 4; i++) {
...
//  Input: the index of the vertex in which incidences are searched:
//  central_vertex_ind,
//         pointer to mesh,
//         pointer to a tetraheron index type: num_incTet.
// Output: by using num_incTet returns the number of non-ghost tetrahedra
// incident in central_vertex,
//         returns an array containing the indices of non-ghost tetrahedra
//         incident in central_vertex.
uint64_t *TetMesh::incident_tetrahedra(const uint32_t central_vertex_ind,
                                       uint64_t *num_incTet) // Mod.1
{
...
// Output: by num_related_tet returns the number of tetrahedra that share with
// first_tet the edge,
//         returns the array containing the indices of those tet_
//  Note. The tetrahedron must have (edge_ends[0],edge_ends[1]) as its side.
uint64_t *TetMesh::ETrelation(const uint32_t *edge_ends,
                              const uint64_t tet0_ind,
                              uint64_t *length_related_tet) const {
  ...
  // move_dir is the face-ID of tet0->tet1, and vrt-ID opposite to that face
  // [wrt tet0] Move from tet0 to tet1
  prec_tet_ind = tet0_ind;
  curr_tet_ind = tet_neigh[4 * tet0_ind + move_dir] >> 2; // tet1

  // other_tet_vrts_ind[1] is the index of the vertex opposite to the face
  // between tet0 and tet1, other_tet_vrts_ind[0] is the index of the common
  // vertex (not on the edge) between tet0 and tet1, other_tet_vrts_ind[0] is
  // the index of the vertex opposite to the face between tet1 and tet2
  for (uint64_t j = 0; j < 4; j++)
    if (tet_node[4 * curr_tet_ind + j] == other_tet_vrts_ind[0]) {
      next_move_dir = j; // is the face-ID of tet1 -> tet2, and vrt-ID opposite
                         // to that face [wrt tet1]
      break;
    }
  ...
  while (curr_tet_ind != tet0_ind) {
    ...
    // Find the vrt-ID of the common vertex (not on the edge) between curr_tet
    // and next_tet [wrt curr_tet].
    uint64_t tmp = tet_neigh[4 * prec_tet_ind + move_dir] & 3;
    // Find the index of the vertex of curr_tet opposite to the face between
    // curr_tet and prec_tet.
    ...
    curr_tet_ind =
        tet_neigh[4 * curr_tet_ind + move_dir] >> 2; // new curr_tet is nex_tet

    // Find the face-ID of the common face between new curr_tet and new prec_tet
    // [wrt new curr_tet].
    for (uint64_t j = 0; j < 4; j++)
  ...
  uint64_t *related_tet =
      (uint64_t *)malloc(sizeof(uint64_t) * num_related_tet);
...
bool TetMesh::vertexNotCoplanar(uint32_t f1, uint32_t f2, uint32_t f3,
                                uint32_t v_id) const {
  if (f3 == INFINITE_VERTEX)
    return true;
  return (orient3d(vertices[f1].coord, vertices[f2].coord, vertices[f3].coord,
                   vertices[v_id].coord) != 0);
}
...
void TetMesh::deleteInVertexTets(uint64_t tet, const uint32_t v_id) {
  ...
  for (start = Del_num_deleted - 1; start < Del_num_deleted; start++) {
    ...
    uint64_t neigh = getIthNeighbor(Neigh, 0);
    if (tet_subdet[neigh + 3] != -1.0) {
      if (vertexNotCoplanar(
              Node[1], Node[2], Node[3],
              v_id)) { // Se v_id non coplanare con la faccia comune
        bnd_push(v_id, Node[1], Node[2], Node[3], Neigh[0]);
      } else {
        Del_deleted[Del_num_deleted++] = neigh;
        tet_subdet[neigh + 3] = -1.0;
...
//         free(vertices); fclose(file); return false; } v->original_index = i;
//     }
//
//     tet_node = (uint32_t*)malloc(nt * sizeof(uint32_t) * 4);
//     for (uint32_t i = 0; i < nt; i++) {
//         uint32_t* nodes = tet_node + i * 4;
//         if (fscanf(file, "4 %u %u %u %u\n", nodes, nodes +1, nodes +2, nodes
//         +3) != 4) { free(vertices); fclose(file); return false; }
//         vertices[nodes[0]].inc_tet = vertices[nodes[1]].inc_tet =
//         vertices[nodes[2]].inc_tet = vertices[nodes[3]].inc_tet = i; if
//         (orient3d(vertices[nodes[0]].coord, vertices[nodes[1]].coord,
//         vertices[nodes[2]].coord, vertices[nodes[3]].coord) < 0)
...
//     }
//
//     // Per ogni tripla con un solo tet, crea un ghost tet e aggiungi 4 triple
//     al vettore size_t num_ghosts = 0; for (int i = 0; i < nt * 4; i++) {
...
//             facets.push_back(tetFacet(f.v[0], f.v[1], INFINITE_VERTEX, last +
//             2)); facets.push_back(tetFacet(f.v[0], f.v[2], INFINITE_VERTEX,
//             last + 1)); facets.push_back(tetFacet(INFINITE_VERTEX, f.v[1],
//             f.v[2], last)); tet_node[last++] = f.v[0]; tet_node[last++] =
//             f.v[1]; tet_node[last++] = f.v[2];
//             // This is ugly!
//             if (orient3d(vertices[f.v[0]].coord, vertices[f.v[1]].coord,
//             vertices[f.v[2]].coord, vertices[tet_node[f.t]].coord) > 0) {
//                 std::swap(tet_node[last - 1], tet_node[last - 2]);
//                 std::swap(facets[facets.size() - 2].t, facets[facets.size() -
//                 3].t);
//             }
...
//     // Le adiacenze dei tet di bordo con i loro ghost si fanno subito qui
//     sopra
//     // Quelle ghost-ghost si fanno riordinando e scorrendo solo la coda del
//     vettore di facce
//
//     // Riordina lexico
//     std::sort(facets.begin(), facets.end());
...
bool TetMesh::loadMEDIT(const char *filename) {
  ...
  do {
    if (!fscanf(file, "%s\n", line))
      ip_error("TetMesh::loadMSH: Could not find Vertices keyword\n");
  } while (strcmp(line, "Vertices"));

  int dummy, nv = 0;
  if (!fscanf(file, "%d\n", &nv))
    ip_error("TetMesh::loadMSH: Could not read num vertices\n");
  ...
  for (int i = 0; i < nv; i++) {
    ...
    if (fscanf(file, "%lf %lf %lf %d\n", crd, crd + 1, crd + 2, &dummy) != 4)
      ip_error("TetMesh::loadMSH: Could not read vertex coords\n");
  }

  int nt;
  if (!fscanf(file, "%s\n", line) || strcmp(line, "Tetrahedra"))
    ip_error("TetMesh::loadMSH: Could not find Tetrahedra keyword\n");
  if (!fscanf(file, "%d\n", &nt))
    ip_error("TetMesh::loadMSH: Could not read num tet\n");
  ...
  for (uint32_t i = 0; i < (uint32_t)nt * 4; i++)
    t_idx[i]--; // Decrease all indexes by one
...
void TetMesh::init(const double *vertex_coords, size_t _num_vertices,
  ...
  for (size_t i = 0; i < _num_tets; i++) {
    ...
    vertices[nodes[0]].inc_tet = vertices[nodes[1]].inc_tet =
        vertices[nodes[2]].inc_tet = vertices[nodes[3]].inc_tet = i;
    if (orient3d(vertices[nodes[0]].coord, vertices[nodes[1]].coord,
                 vertices[nodes[2]].coord, vertices[nodes[3]].coord) < 0)
      std::swap(nodes[0], nodes[1]);
  }
  ...
  // Per ogni tripla con un solo tet, crea un ghost tet e aggiungi 4 triple al
  // vettore
  size_t num_ghosts = 0;
  for (size_t i = 0; i < tet_num * 4; i++) {
    const tetFacet &f = facets[i];
