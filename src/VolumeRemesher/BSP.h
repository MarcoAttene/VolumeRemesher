...
public:
    ...
    BSPface(uint32_t m_v1, uint32_t m_v2, uint32_t m_v3, uint64_t c1, uint64_t c2, COLOUR_T _colour)
    {
        meshVertices[0] = m_v1;
        meshVertices[1] = m_v2;
        meshVertices[2] = m_v3;
...
class BSPcell
{ // A convex polyhedron defined by the intersection of a
...
public:
    ...
    BSPcell(const vector<uint64_t>& cell_faces, const vector<uint32_t>& intersect_constrs)
    {
        faces.assign(cell_faces.begin(), cell_faces.end());
...
public:
    ...
    void cell_VFrelation(const BSPcell& cell, uint32_t v, vector<uint64_t>& v_incFaces_ind);
    void
    COMPL_cell_VFrelation(const BSPcell& cell, uint32_t v, vector<uint64_t>& v_NOT_incFaces_ind);
    ...
    void vrts_orient_wrtPlane(
        ...
        uint32_t count);
    inline void
    count_vrt_orBin(const vector<uint32_t>& inds, uint32_t* pos, uint32_t* neg, uint32_t* zero);
    inline bool constraint_innerIntersects_edge(
        const BSPedge& edge,
        const vector<uint32_t>& cell_vrts);
    inline bool constraint_innerIntersects_face(const vector<uint32_t>& face_vrts);
    bool coplanar_constraint_innerIntersects_face(
        const vector<uint64_t>& face_edges,
    ...
    inline uint64_t
    add_tetFace(uint32_t v0, uint32_t v1, uint32_t v2, uint64_t cell_ind, uint64_t adjCell_ind);
    inline bool tet_face_isNew(uint64_t tet_ind, uint64_t adjTet_ind, uint64_t adjCell_ind);
    inline void fill_face_colour(
        uint64_t tet_ind,
        uint64_t face_ind,
    ...
    // BSPsubdivision
    inline void move_edge(uint64_t edge_face_ind, uint64_t face_ind, uint64_t newFace_ind);
    inline void move_face(uint64_t face_cell_ind, uint64_t cell_ind, uint64_t newCell_ind);
    ...
    void facesPartition(uint64_t cell_ind, uint64_t newCell_ind, const vector<uint32_t>& cell_vrts);
    void constraintsPartition(
        uint32_t ref_constr,
    ...
    void add_edges_toCommFaceEdges(BSPface& face, const vector<uint64_t>& edges_ind);
    void add_commonFace(
        uint32_t constr,
        uint64_t cell_ind,
    ...
    void
    find_coplanar_constraints(uint64_t cell_ind, uint32_t constr, vector<uint32_t>& coplanar_c);
    ...
    bool
    is_baricenter_inFace(const BSPface& face, const explicitPoint3D& face_center, int max_normComp);
...
/// <param name="npts_A"></param>
/// <param name="tri_idx_A"></param>
/// <param name="ntri_A"></param>
/// <param name="tetfile_name"></param>
/// <param name="free_mem"></param>
/// <param name="verbose"></param>
/// <returns></returns>
BSPcomplex* remakePolyhedralMesh(
    const double* coords_A,
    uint32_t npts_A,
    const uint32_t* tri_idx_A,
    uint32_t ntri_A,
