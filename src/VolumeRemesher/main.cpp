...
void read_OFF_file(const char *filename, double **vertices_p, uint32_t *npts,
  ...
  if (verbose)
    printf("file %s contains %d vertices and %d constraints (triangles)\n",
           filename, *npts, *ntri);
...
void read_TET_file(const char *filename, double **vertices_p, uint32_t *npts,
  ...
  if (!fscanf(file, "%d vertices\n", &nv))
    ip_error(
        "read_TET_file: FATAL ERROR\ncannot read 1st line of input file\n");
  if (!fscanf(file, "%d tets\n", &nt))
    ip_error(
        "read_TET_file: FATAL ERROR\ncannot read 2nd line of input file\n");

  if (verbose)
    printf("file %s contains %u vertices and %u tetrahedra\n", filename, nv,
           nt);
...
void read_MEDIT_file(const char *filename, double **vertices_p, uint32_t *npts,
  ...
  int dummy, nv = 0;
  if (!fscanf(file, "%d\n", &nv))
    ip_error("read_MEDIT_file: Could not read num vertices\n");
  ...
  if (!fscanf(file, "%d\n", &nt))
    ip_error("read_MEDIT_file: Could not read num tet\n");
...
int main(int argc, char **argv) {
#ifdef NDEBUG
  if (argc < 2) {
    printf("\nUsage: mesh_generator [-v | -l | -s | -b | -t] inputfile_A.off "
           "[bool_opcode inputfile_B.off]\n\n"
           "Defines the volume enclosed by the input OFF file(s) and saves a "
           "volume mesh to 'volume.msh'\n\n"
  ...
  const bool file_B_is_tet =
      fileB_name != NULL &&
      !strcmp(fileB_name + strlen(fileB_name) - 4, ".tet");
  const bool file_B_is_msh =
      fileB_name != NULL &&
      !strcmp(fileB_name + strlen(fileB_name) - 4, ".msh");

  bool embedsurf = (file_B_is_tet || file_B_is_msh);
  if (embedsurf)
    two_input = false;

  read_OFF_file(fileA_name, &coords_A, &ncoords_A, &tri_idx_A, &ntriidx_A,
                verbose);
  if (two_input)
  ...
  if (embedsurf) {
    if (file_B_is_tet)
      read_TET_file(fileB_name, &coords_B, &ncoords_B, &tri_idx_B, &ntriidx_B,
                    verbose);
    else
      read_MEDIT_file(fileB_name, &coords_B, &ncoords_B, &tri_idx_B, &ntriidx_B,
                      verbose);

    // const std::vector<double> tri_vrt_coords(coords_A, coords_A + ncoords_A *
    // 3); const std::vector<uint32_t> triangle_indexes(tri_idx_A, tri_idx_A +
    // ntriidx_A * 3); const std::vector<double> tet_vrt_coords(coords_B,
    // coords_B + ncoords_B * 3); const std::vector<uint32_t>
    // tet_indexes(tri_idx_B, tri_idx_B + ntriidx_B * 4);
    ...
    complex = remakePolyhedralMesh(coords_A, ncoords_A, tri_idx_A, ntriidx_A,
                                   coords_B, ncoords_B, tri_idx_B, ntriidx_B,
                                   verbose);
    free(coords_A);
    free(tri_idx_A);
  } else
    complex = makePolyhedralMesh(fileA_name, coords_A, ncoords_A, tri_idx_A,
                                 ntriidx_A, fileB_name, coords_B, ncoords_B,
                                 tri_idx_B, ntriidx_B, bool_opcode, true,
                                 verbose, logging);
  ...
  if (blackfaces)
    ...
    complex->saveMesh((triangulate) ? ("volume.tet") : ("volume.msh"),
                      bool_opcode, triangulate);
  printf("Done.\n");
